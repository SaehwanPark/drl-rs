...
- `drl-render`: deterministic `PresentationStep`, `RenderScene`, target
  ...
  only; GPU ownership is in the WASM shell. `drl-web` carries fair, visibility-
  filtered spans through each successful `PresentationStep`.
...
- `PresentationStep::effects` is computed at the command boundary from the
  returned event list and before/after visible actor sets; rejected commands
  produce no step and no effects.
