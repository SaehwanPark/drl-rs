...
- **Role**: Build-time conversion boundary for legacy content. The current
  ...
  synchronizes complete record ID catalogs, scalar-only evidence fields,
  structured migration gaps, and exact pinned source digests with the Rust
  descriptive catalog; runtime Lua remains prohibited.
