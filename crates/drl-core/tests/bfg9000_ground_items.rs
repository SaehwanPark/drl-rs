...
fn first_destructive_cell(game: &Game, center: Position) -> Position {
  first_cell_with_roll(game, center, |damage| damage > 10)
}

fn first_cell_with_roll(
  game: &Game,
  center: Position,
  ...
  radius_eight_blast_positions(game.world().map(), center)
    .into_iter()
    .skip(1)
    .find(|_| predicate(roll_explosion_damage(&mut expected_rng)))
    .expect("a radius-8 blast must contain a matching 10d6 roll")
}
...
#[test]
fn bfg9000_destroys_lowest_id_ordinary_ground_item_above_threshold() {
  let center = Position::new(14, 12);
  let mut game = equipped_bfg9000(30_000);
  let center_id = game
    .world_mut()
...
#[test]
fn bfg9000_ground_item_effect_replays_with_identical_events_and_state() {
  ...
  let mut setup = ReplayLog::new(30_000, 12, 12, player_position).with_player_config(player_config);
  ...
  let expected_events = direct
    .step(command)
    .expect("direct BFG fire should resolve");
  ...
  let (replayed, replay_events) = ReplayEngine::run(&replay).expect("BFG replay should resolve");
  assert_eq!(replayed, direct);
  assert_eq!(replay_events, expected_events);
  assert!(ReplayEngine::verify_determinism(&replay).expect("BFG replay should be deterministic"));
}
