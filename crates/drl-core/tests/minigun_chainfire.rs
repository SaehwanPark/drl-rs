...
#[test]
fn minigun_second_chainfire_emits_eight_projectiles_and_advances_state() {
  let mut game = equipped_minigun(2_260);
  let target = Position::new(5, 2);
  let target_id = game
    .world_mut()
  ...
  let weapon = game.world().player().unwrap().equipment().weapon().unwrap();
  assert_eq!(weapon.weapon_properties().unwrap().current_clip, 186);
  ...
  assert_eq!(
    events
      ...
      .count(),
    8,
    "Minigun second chainfire must resolve eight ordered projectiles"
  );
}
...
#[test]
fn minigun_second_chainfire_below_eight_round_cost_rejection_is_atomic() {
  let mut game = equipped_minigun(2_341);
  let target = Position::new(5, 2);
  game
    .world_mut()
...
#[test]
fn minigun_chainfire_replay_preserves_six_then_eight_projectile_bursts_deterministically() {
  let player_start = Position::new(5, 5);
  let mut replay =
    ReplayLog::new(2_347, 12, 12, player_start).with_player_config(PlayerSpawnConfig {
      hp: 50,
      max_hp: 50,
      speed: 100,
      initial_items: Vec::new(),
