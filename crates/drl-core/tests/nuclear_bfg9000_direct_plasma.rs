...
fn direct_attack_damage(events: &[GameEvent], target_id: drl_protocol::EntityId) -> (u32, u32) {
  let raw = events
    ...
    .expect("Nuclear BFG 9000 direct hit should resolve");
  let applied = events
    .iter()
    .find_map(|event| match event {
    ...
    .expect("Nuclear BFG 9000 direct damage should be typed Plasma");
  (raw, applied)
}

#[test]
fn nuclear_bfg9000_direct_hit_is_typed_plasma_and_blue_armor_mitigates() {
  let seed = 42_003;
  let target_position = Position::new(7, 12);
  let mut plain = equipped_nuclear_bfg(seed);
  let mut armored = equipped_nuclear_bfg(seed);
  ...
  let plain_events = plain
    ...
    .expect("unarmored Nuclear BFG 9000 fire should resolve");
  let armored_events = armored
    .step(Command::AttackRanged(target_position))
    .expect("Blue Armor Nuclear BFG 9000 fire should resolve");
...
#[test]
fn nuclear_bfg9000_direct_plasma_replays_deterministically() {
  let seed = 42_004;
  ...
  stale.metadata.gameplay_semantics_version = 136;
