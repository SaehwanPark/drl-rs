...
fn direct_volley_damage(
  ...
  assert_eq!(
    ...
    "Nuclear Plasma Rifle volley must resolve the expected attack count"
  );
  assert!(
    !raw.is_empty(),
    "fixed seed should include a successful Nuclear Plasma Rifle hit"
  );
  assert_eq!(
    applied.len(),
    raw.len(),
    "each successful Nuclear Plasma Rifle direct hit must emit one typed Plasma event"
  );
  (raw, applied)
}

#[test]
fn nuclear_plasma_direct_volley_is_typed_plasma_and_blue_armor_mitigates() {
  let seed = 44_003;
  let target_position = Position::new(7, 12);
  let mut plain = equipped_nuclear_plasma_rifle(seed);
  let mut armored = equipped_nuclear_plasma_rifle(seed);
  ...
  let plain_events = plain
    ...
    .expect("unarmored Nuclear Plasma Rifle fire should resolve");
  let armored_events = armored
    .step(Command::AttackRanged(target_position))
    .expect("Blue Armor Nuclear Plasma Rifle fire should resolve");
  let (raw_damage, plain_damage) = direct_volley_damage(&plain_events, plain_target_id, 6);
  let (armored_raw_damage, armored_damage) =
    direct_volley_damage(&armored_events, armored_target_id, 6);
...
#[test]
fn nuclear_plasma_chainfire_direct_hits_are_typed_plasma() {
  let mut game = equipped_nuclear_plasma_rifle(44_004);
  let target_position = Position::new(7, 12);
  let target_id = configure_direct_target(&mut game, target_position);

  let events = game
    .step(Command::AttackRangedChainfire(target_position))
    .expect("first Nuclear Plasma chainfire burst should resolve");

  let typed_direct_hits = events
    .iter()
    .filter(|event| {
      matches!(
        event,
    ...
    .count();
  let successful_direct_hits = events
    .iter()
    .filter(|event| {
      matches!(
        event,
    ...
    .count();
  assert!(successful_direct_hits > 0);
  assert_eq!(typed_direct_hits, successful_direct_hits);
}

#[test]
fn nuclear_plasma_direct_plasma_replays_deterministically() {
  let seed = 44_005;
  ...
  let (first_game, first_events) =
    ReplayEngine::run(&replay).expect("direct Nuclear Plasma replay should run");
  let (second_game, second_events) =
    ReplayEngine::run(&replay).expect("direct Nuclear Plasma replay should repeat");
  ...
  let target_id = first_game
    ...
    .id();
  let (_, applied) = direct_volley_damage(&first_events, target_id, 6);
  assert!(!applied.is_empty());

  let mut stale = replay;
  stale.metadata.gameplay_semantics_version = 139;
