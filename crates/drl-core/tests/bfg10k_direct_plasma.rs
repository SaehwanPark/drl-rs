...
fn direct_volley_damage(
  ...
  let applied = events
    ...
    .collect::<Vec<_>>();
  assert_eq!(raw.len(), 5, "BFG 10K volley must resolve five direct hits");
  assert_eq!(
    applied.len(),
    5,
    "BFG 10K direct damage must emit five typed Plasma events"
  );
  (raw, applied)
}

#[test]
fn bfg10k_direct_volley_is_typed_plasma_and_blue_armor_mitigates() {
  let seed = 43_003;
  let target_position = Position::new(7, 12);
  let mut plain = equipped_bfg10k(seed);
  let mut armored = equipped_bfg10k(seed);
  ...
  let plain_events = plain
    ...
    .expect("unarmored BFG 10K fire should resolve");
  let armored_events = armored
    .step(Command::AttackRanged(target_position))
    .expect("Blue Armor BFG 10K fire should resolve");
...
#[test]
fn bfg10k_direct_plasma_replays_deterministically() {
  let seed = 43_004;
  ...
  stale.metadata.gameplay_semantics_version = 137;
