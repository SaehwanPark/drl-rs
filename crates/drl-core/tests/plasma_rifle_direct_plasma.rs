...
fn direct_volley_damage(
  ...
  assert_eq!(
    attack_count, 6,
    "Plasma Rifle volley must resolve six attacks"
  );
  assert!(
    !raw.is_empty(),
  ...
  assert_eq!(
    applied.len(),
    raw.len(),
    "each successful Plasma Rifle direct hit must emit one typed Plasma event"
  );
  (raw, applied)
}

#[test]
fn plasma_rifle_direct_volley_is_typed_plasma_and_blue_armor_mitigates() {
  let seed = 44_003;
  let target_position = Position::new(7, 12);
  let mut plain = equipped_plasma_rifle(seed);
  let mut armored = equipped_plasma_rifle(seed);
  ...
  let plain_events = plain
    ...
    .expect("unarmored Plasma Rifle fire should resolve");
  let armored_events = armored
    .step(Command::AttackRanged(target_position))
    .expect("Blue Armor Plasma Rifle fire should resolve");
...
#[test]
fn plasma_rifle_chainfire_direct_hits_are_typed_plasma() {
  let mut game = equipped_plasma_rifle(44_004);
  let target_position = Position::new(7, 12);
  let target_id = configure_direct_target(&mut game, target_position);

  let events = game
    .step(Command::AttackRangedChainfire(target_position))
    .expect("first Plasma Rifle chainfire burst should resolve");
...
#[test]
fn plasma_rifle_direct_plasma_replays_deterministically() {
  let seed = 44_005;
  ...
  let target_id = first_game
    ...
    .id();
  let (_, applied) = direct_volley_damage(&first_events, target_id);
  assert!(!applied.is_empty());

  let mut stale = replay;
  stale.metadata.gameplay_semantics_version = 138;
