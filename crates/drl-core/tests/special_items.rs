...
fn chaingun_fifteenth_chainfire_level_uses_the_whole_rule() {
  ...
  game
    .step(Command::AttackRangedChainfire(target))
    .expect("thirteenth chainfire burst");
  game
    .step(Command::Reload)
    .expect("reload before fourteenth chainfire burst");
  game
    .step(Command::AttackRangedChainfire(target))
...
#[test]
fn chaingun_thirteenth_chainfire_after_twelfth_emits_six_projectiles_and_advances_state() {
  let (mut game, _weapon_id) = equipped_chaingun(2_278);
  let target = Position::new(5, 2);
  let target_id = game
    .world_mut()
    .spawn_monster(target, "Static Target", 10_000, 0, (1, 6))
...
#[test]
fn chaingun_thirteenth_chainfire_below_six_round_cost_rejection_is_atomic() {
  let (mut game, _weapon_id) = equipped_chaingun(2_280);
  let target = Position::new(5, 2);
  game
    .world_mut()
    .spawn_monster(target, "Static Target", 10_000, 0, (1, 6))
...
#[test]
fn chaingun_fourteenth_chainfire_after_thirteenth_emits_six_projectiles_and_advances_state() {
  let (mut game, _weapon_id) = equipped_chaingun(2_282);
  let target = Position::new(5, 2);
  let target_id = game
    .world_mut()
    .spawn_monster(target, "Static Target", 10_000, 0, (1, 6))
  ...
  game
    ...
    .unwrap()
    .inventory_mut()
    .add_item(Item::ammo_9mm(ammo_id, 50))
    .unwrap();
  for _ in 0..7 {
    game
      .step(Command::AttackRangedChainfire(target))
  ...
  game
    .step(Command::Reload)
    .expect("reload before fourteenth burst");
...
#[test]
fn chaingun_fourteenth_chainfire_below_six_round_cost_rejection_is_atomic() {
  let (mut game, _weapon_id) = equipped_chaingun(2_284);
  let target = Position::new(5, 2);
  game
    .world_mut()
    .spawn_monster(target, "Static Target", 10_000, 0, (1, 6))
...
#[test]
fn chaingun_chainfire_replay_preserves_thirteenth_level_after_twelfth_deterministically() {
  let player_start = Position::new(5, 5);
  let mut replay =
    ReplayLog::new(2_278, 12, 12, player_start).with_player_config(PlayerSpawnConfig {
      hp: 50,
      max_hp: 50,
      speed: 100,
      initial_items: vec![ItemSpawnKind::Ammo9mm(40)],
  ...
  let (game, events) =
    ReplayEngine::run(&replay).expect("Chaingun thirteenth chainfire replay should run");
  let weapon_properties = game
    .world()
    .player()
    .unwrap()
    ...
    .weapon_properties()
    .unwrap();
  assert_eq!(weapon_properties.current_clip, 4);
  assert_eq!(weapon_properties.chainfire_level, 13);
  assert_eq!(
    events
      .iter()
      .filter(|event| matches!(
...
#[test]
fn chaingun_chainfire_replay_preserves_fourteenth_level_after_thirteenth_deterministically() {
  let player_start = Position::new(5, 5);
  let mut replay =
    ReplayLog::new(2_286, 12, 12, player_start).with_player_config(PlayerSpawnConfig {
      hp: 50,
      max_hp: 50,
      speed: 100,
  ...
  let (game, events) =
    ReplayEngine::run(&replay).expect("Chaingun fourteenth chainfire replay should run");
  let weapon_properties = game
    .world()
    .player()
    .unwrap()
    ...
    .unwrap();
  assert_eq!(weapon_properties.current_clip, 34);
  assert_eq!(weapon_properties.chainfire_level, 14);
  assert_eq!(
    events
      .iter()
      .filter(|event| matches!(
