...
#[test]
fn bfg10k_fifteenth_chainfire_vertical_scenario_preserves_replay() {
  let mut scenario = Scenario::from_ascii(
    "Bfg10kFifteenthChainfireVertical",
    "BFG 10K carries its fifteenth-stage chainfire encounter across reloads",
    "########\n#@....h#\n#......#\n########\n",
  )
  .unwrap();
  scenario.seed = 0;
  ...
  let properties = weapon.weapon_properties().unwrap();
  assert_eq!(properties.current_clip, 15);
  assert_eq!(properties.chainfire_level, 15);
  assert_eq!(replay.commands, commands);
