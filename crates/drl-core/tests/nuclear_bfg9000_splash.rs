...
#[test]
fn nuclear_bfg_splash_preserves_knockback_before_lethal_death_drop() {
  let center = Position::new(14, 12);
  let victim_position = Position::new(14, 13);
  let mut selected = None;
  for seed in 31_001..31_040 {
    let mut game = equipped_nuclear_bfg(seed);
    let center_id = game
      .world_mut()
  ...
  let (game, events, center_id, victim_id) =
    selected.expect("a fixed seed should exercise Nuclear BFG 9000 knockback");
