...
fn set_bfg10k_clip(game: &mut Game, current_clip: u32) {
  let player_id = game.world().player_id().unwrap();
  game
    .world_mut()
    .get_actor_mut(player_id)
...
fn advance_bfg10k_chainfire_to_level(
  game: &mut Game,
  target: Position,
  target_level: u8,
) -> Vec<GameEvent> {
  ...
  for current_level in 2..target_level {
    set_bfg10k_clip(game, if current_level == 2 { 35 } else { 50 });
    events = game
      .step(Command::AttackRangedChainfire(target))
      .unwrap_or_else(|error| {
        panic!(
          "BFG 10K chainfire burst advancing from level {current_level} should be accepted: {error}"
        )
      });
  }
  events
...
#[test]
fn bfg10k_sixteenth_chainfire_emits_seven_exact_hits_and_advances_state() {
  let mut game = equipped_bfg10k(2_738);
  let target = Position::new(8, 2);
  let target_id = game
    .world_mut()
    .spawn_monster(target, "Static Target", 10_000, 0, (1, 7))
  ...
  let events = advance_bfg10k_chainfire_to_level(&mut game, target, 16);

  let weapon = game.world().player().unwrap().equipment().weapon().unwrap();
  let props = weapon.weapon_properties().unwrap();
  assert_eq!(props.current_clip, 15);
  assert_eq!(props.chainfire_level, 16);
  assert_eq!(ranged_events(&events, player_id), 7);
  assert_eq!(bfg10k_schedules(&events, target_id), 7);
}

#[test]
fn bfg10k_seventeenth_chainfire_emits_seven_exact_hits_and_advances_state() {
  let mut game = equipped_bfg10k(2_740);
  let target = Position::new(8, 2);
  let target_id = game
    .world_mut()
    .spawn_monster(target, "Static Target", 10_000, 0, (1, 7))
  ...
  let events = advance_bfg10k_chainfire_to_level(&mut game, target, 17);

  let weapon = game.world().player().unwrap().equipment().weapon().unwrap();
  let props = weapon.weapon_properties().unwrap();
  assert_eq!(props.current_clip, 15);
  assert_eq!(props.chainfire_level, 17);
  assert_eq!(ranged_events(&events, player_id), 7);
  assert_eq!(bfg10k_schedules(&events, target_id), 7);
}

#[test]
fn bfg10k_eighteenth_chainfire_emits_seven_exact_hits_and_advances_state() {
  let mut game = equipped_bfg10k(2_742);
  let target = Position::new(8, 2);
  let target_id = game
    .world_mut()
    .spawn_monster(target, "Static Target", 10_000, 0, (1, 7))
  ...
  let events = advance_bfg10k_chainfire_to_level(&mut game, target, 18);

  let weapon = game.world().player().unwrap().equipment().weapon().unwrap();
  let props = weapon.weapon_properties().unwrap();
  assert_eq!(props.current_clip, 15);
  assert_eq!(props.chainfire_level, 18);
  assert_eq!(ranged_events(&events, player_id), 7);
  assert_eq!(bfg10k_schedules(&events, target_id), 7);
}
...
#[test]
fn bfg10k_twentieth_chainfire_emits_seven_exact_hits_and_advances_state() {
  let mut game = equipped_bfg10k(2_746);
  let target = Position::new(8, 2);
  let target_id = game
    .world_mut()
    .spawn_monster(target, "Static Target", 10_000, 0, (1, 7))
  ...
  let events = advance_bfg10k_chainfire_to_level(&mut game, target, 20);

  let weapon = game.world().player().unwrap().equipment().weapon().unwrap();
  let props = weapon.weapon_properties().unwrap();
  assert_eq!(props.current_clip, 15);
  assert_eq!(props.chainfire_level, 20);
  assert_eq!(ranged_events(&events, player_id), 7);
  assert_eq!(bfg10k_schedules(&events, target_id), 7);
}

#[test]
fn bfg10k_twenty_first_chainfire_emits_seven_exact_hits_and_advances_state() {
  let mut game = equipped_bfg10k(2_748);
  let target = Position::new(8, 2);
  let target_id = game
    .world_mut()
    .spawn_monster(target, "Static Target", 10_000, 0, (1, 7))
  ...
  let events = advance_bfg10k_chainfire_to_level(&mut game, target, 21);

  let weapon = game.world().player().unwrap().equipment().weapon().unwrap();
  let props = weapon.weapon_properties().unwrap();
  assert_eq!(props.current_clip, 15);
  assert_eq!(props.chainfire_level, 21);
  assert_eq!(ranged_events(&events, player_id), 7);
  assert_eq!(bfg10k_schedules(&events, target_id), 7);
}
...
fn bfg10k_twenty_second_chainfire_under_supply_remains_atomic() {
  ...
  game
    .step(Command::AttackRangedChainfire(third_target))
    .expect("sixteenth BFG 10K chainfire burst");
  set_bfg10k_clip(&mut game, 50);
  game
    .step(Command::AttackRangedChainfire(third_target))
    .expect("seventeenth BFG 10K chainfire burst");
  set_bfg10k_clip(&mut game, 50);
  game
    .step(Command::AttackRangedChainfire(third_target))
  ...
  game
    .step(Command::AttackRangedChainfire(third_target))
    .expect("twentieth BFG 10K chainfire burst");
  set_bfg10k_clip(&mut game, 50);
  game
    .step(Command::AttackRangedChainfire(third_target))
    .expect("twenty-first BFG 10K chainfire burst");
  let before = game.clone();
...
#[test]
fn bfg10k_sixteenth_chainfire_below_thirty_five_cell_cost_rejection_is_atomic() {
  let mut game = equipped_bfg10k(2_739);
  let target = Position::new(8, 2);
  game
    .world_mut()
    .spawn_monster(target, "Static Target", 10_000, 0, (1, 7))
  ...
  advance_bfg10k_chainfire_to_level(&mut game, target, 15);
...
#[test]
fn bfg10k_seventeenth_chainfire_below_thirty_five_cell_cost_rejection_is_atomic() {
  let mut game = equipped_bfg10k(2_741);
  let target = Position::new(8, 2);
  game
    .world_mut()
    .spawn_monster(target, "Static Target", 10_000, 0, (1, 7))
  ...
  advance_bfg10k_chainfire_to_level(&mut game, target, 16);
  set_bfg10k_clip(&mut game, 34);
  let before = game.clone();
...
#[test]
fn bfg10k_eighteenth_chainfire_below_thirty_five_cell_cost_rejection_is_atomic() {
  let mut game = equipped_bfg10k(2_743);
  let target = Position::new(8, 2);
  game
    .world_mut()
    .spawn_monster(target, "Static Target", 10_000, 0, (1, 7))
...
#[test]
fn bfg10k_twentieth_chainfire_below_thirty_five_cell_cost_rejection_is_atomic() {
  let mut game = equipped_bfg10k(2_747);
  let target = Position::new(8, 2);
  game
    .world_mut()
    .spawn_monster(target, "Static Target", 10_000, 0, (1, 7))
  ...
  advance_bfg10k_chainfire_to_level(&mut game, target, 19);
  set_bfg10k_clip(&mut game, 34);
  let before = game.clone();
...
#[test]
fn bfg10k_twenty_first_chainfire_below_thirty_five_cell_cost_rejection_is_atomic() {
  let mut game = equipped_bfg10k(2_749);
  let target = Position::new(8, 2);
  game
    .world_mut()
    .spawn_monster(target, "Static Target", 10_000, 0, (1, 7))
  ...
  advance_bfg10k_chainfire_to_level(&mut game, target, 20);
  set_bfg10k_clip(&mut game, 34);
  let before = game.clone();
...
#[test]
fn bfg10k_sixteenth_chainfire_replay_is_deterministic() {
  let player_start = Position::new(5, 5);
  let mut replay =
    ReplayLog::new(2_738, 12, 12, player_start).with_player_config(PlayerSpawnConfig {
      hp: 50,
      max_hp: 50,
      speed: 100,
  ...
  let (game, events) =
    ReplayEngine::run(&replay).expect("BFG 10K sixteenth chainfire replay should run");
  let weapon = game.world().player().unwrap().equipment().weapon().unwrap();
  let props = weapon.weapon_properties().unwrap();
  assert_eq!(props.current_clip, 15);
  assert_eq!(props.chainfire_level, 16);
...
#[test]
fn bfg10k_seventeenth_chainfire_replay_is_deterministic() {
  let player_start = Position::new(5, 5);
  let mut replay =
    ReplayLog::new(2_740, 12, 12, player_start).with_player_config(PlayerSpawnConfig {
      hp: 50,
      max_hp: 50,
      speed: 100,
  ...
  let (game, events) =
    ReplayEngine::run(&replay).expect("BFG 10K seventeenth chainfire replay should run");
  let weapon = game.world().player().unwrap().equipment().weapon().unwrap();
  let props = weapon.weapon_properties().unwrap();
  assert_eq!(props.current_clip, 15);
  assert_eq!(props.chainfire_level, 17);
...
#[test]
fn bfg10k_eighteenth_chainfire_replay_is_deterministic() {
  let player_start = Position::new(5, 5);
  let mut replay =
    ReplayLog::new(2_742, 12, 12, player_start).with_player_config(PlayerSpawnConfig {
      hp: 50,
      max_hp: 50,
      speed: 100,
  ...
  let (game, events) =
    ReplayEngine::run(&replay).expect("BFG 10K eighteenth chainfire replay should run");
  let weapon = game.world().player().unwrap().equipment().weapon().unwrap();
  let props = weapon.weapon_properties().unwrap();
  assert_eq!(props.current_clip, 15);
  assert_eq!(props.chainfire_level, 18);
...
#[test]
fn bfg10k_twentieth_chainfire_replay_is_deterministic() {
  let player_start = Position::new(5, 5);
  let mut replay =
    ReplayLog::new(2_746, 12, 12, player_start).with_player_config(PlayerSpawnConfig {
      hp: 50,
      max_hp: 50,
      speed: 100,
  ...
  let (game, events) =
    ReplayEngine::run(&replay).expect("BFG 10K twentieth chainfire replay should run");
  let weapon = game.world().player().unwrap().equipment().weapon().unwrap();
  let props = weapon.weapon_properties().unwrap();
  assert_eq!(props.current_clip, 15);
  assert_eq!(props.chainfire_level, 20);
...
#[test]
fn bfg10k_twenty_first_chainfire_replay_is_deterministic() {
  let player_start = Position::new(5, 5);
  let mut replay =
    ReplayLog::new(2_748, 12, 12, player_start).with_player_config(PlayerSpawnConfig {
      hp: 50,
      max_hp: 50,
      speed: 100,
  ...
  let (game, events) =
    ReplayEngine::run(&replay).expect("BFG 10K twenty-first chainfire replay should run");
  let weapon = game.world().player().unwrap().equipment().weapon().unwrap();
  let props = weapon.weapon_properties().unwrap();
  assert_eq!(props.current_clip, 15);
  assert_eq!(props.chainfire_level, 21);
