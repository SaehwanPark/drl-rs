...
mod tests {
  ...
  fn test_replay_validation_rejects_incompatible_semantics() {
    ...
    // Version 49 predates the Super Shotgun's two-projectile volley and must
    // not be interpreted by the version-50 engine without migration.
    assert_eq!(drl_protocol::CURRENT_GAMEPLAY_SEMANTICS_VERSION, 50);
    assert_eq!(
      drl_protocol::CURRENT_RNG_SAMPLING_SEMANTICS_VERSION,
      crate::rng::RNG_SAMPLING_SEMANTICS_VERSION
    );
    replay.metadata.gameplay_semantics_version = 49;
