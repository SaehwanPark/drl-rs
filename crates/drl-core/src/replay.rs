...
mod tests {
  ...
  fn test_replay_validation_rejects_incompatible_semantics() {
    ...
    // Version 61 predates Anti-Freak Jackal splash fanout and must not be
    // interpreted by the version-62 engine without migration.
    assert_eq!(drl_protocol::CURRENT_GAMEPLAY_SEMANTICS_VERSION, 62);
    assert_eq!(
      drl_protocol::CURRENT_RNG_SAMPLING_SEMANTICS_VERSION,
      crate::rng::RNG_SAMPLING_SEMANTICS_VERSION
    );
    replay.metadata.gameplay_semantics_version = 61;
