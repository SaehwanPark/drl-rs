...
mod tests {
  ...
  fn test_replay_validation_rejects_incompatible_semantics() {
    ...
    // Version 91 predates BFG 10K's fourth-level chainfire effect and must
    // not be interpreted by the version-92 engine.
    assert_eq!(drl_protocol::CURRENT_GAMEPLAY_SEMANTICS_VERSION, 92);
    assert_eq!(
      drl_protocol::CURRENT_RNG_SAMPLING_SEMANTICS_VERSION,
