...
mod tests {
  ...
  fn test_replay_validation_rejects_incompatible_semantics() {
    ...
    // Version 45 predates the Null Pointer's ten-cell cost and must not be
    // interpreted by the version-46 engine without an explicit migration.
    assert_eq!(drl_protocol::CURRENT_GAMEPLAY_SEMANTICS_VERSION, 46);
    assert_eq!(
      drl_protocol::CURRENT_RNG_SAMPLING_SEMANTICS_VERSION,
      crate::rng::RNG_SAMPLING_SEMANTICS_VERSION
    );
    replay.metadata.gameplay_semantics_version = 45;
