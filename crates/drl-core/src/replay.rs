...
mod tests {
  ...
  fn test_replay_validation_rejects_incompatible_semantics() {
    ...
    // Version 67 predates deterministic chainfire completion after a lethal
    // target and must not be interpreted by the version-68 engine.
    assert_eq!(drl_protocol::CURRENT_GAMEPLAY_SEMANTICS_VERSION, 68);
    assert_eq!(
      drl_protocol::CURRENT_RNG_SAMPLING_SEMANTICS_VERSION,
      crate::rng::RNG_SAMPLING_SEMANTICS_VERSION
    );
    replay.metadata.gameplay_semantics_version = 67;
