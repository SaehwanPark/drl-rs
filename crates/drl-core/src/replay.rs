...
mod tests {
  ...
  fn test_replay_validation_rejects_incompatible_semantics() {
    ...
    // Version 81 predates Nuclear Plasma Rifle's second-level chainfire effect
    // and must not be interpreted by the version-82 engine.
    assert_eq!(drl_protocol::CURRENT_GAMEPLAY_SEMANTICS_VERSION, 82);
    assert_eq!(
      drl_protocol::CURRENT_RNG_SAMPLING_SEMANTICS_VERSION,
