...
pub const fn item_colorization_tint(archetype: ItemArchetype) -> [u8; 4] {
  match archetype {
    ...
    ItemArchetype::RedArmor => [255, 0, 0, 255],
...
pub fn effects_for_events(events: &[GameEvent]) -> Vec<PresentationEffect> {
  events
    ...
    .filter_map(|event| match event {
      ...
      GameEvent::MegaBusterMorphed { .. } | GameEvent::MegaBusterExplosionScheduled { .. } => None,
...
fn event_entity_ids(event: &GameEvent) -> [Option<EntityId>; 2] {
  match event {
    ...
    | GameEvent::RocketLauncherExplosionScheduled { entity_id, .. }
    | GameEvent::RevenantsLauncherExplosionScheduled { entity_id, .. }
    | GameEvent::MissileLauncherExplosionScheduled { entity_id, .. }
...
mod tests {
  ...
  fn item_colorization_tint_maps_verified_armor_and_phase_items() {
    ...
    assert_eq!(
      item_colorization_tint(ItemArchetype::RedArmor),
      [255, 0, 0, 255]
    );
    assert_eq!(
