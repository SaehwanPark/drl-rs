...
"#;
  ...
  fn scene_position(scene: &RenderScene, x: i32, y: i32) -> (f32, f32, f32, f32) {
    let width = scene.map_width.max(1) as f32;
    let height = scene.map_height.max(1) as f32;
    let left = -1.0 + 2.0 * (x.max(0) as f32) / width;
    let right = -1.0 + 2.0 * ((x + 1).max(1) as f32) / width;
    let top = 1.0 - 2.0 * (y.max(0) as f32) / height;
    let bottom = 1.0 - 2.0 * ((y + 1).max(1) as f32) / height;
    (left, bottom, right, top)
  }

  fn scene_vertices(scene: &RenderScene) -> Vec<u8> {
    ...
    for tile in &scene.tiles {
      ...
      let (left, bottom, right, top) = scene_position(scene, tile.position.x, tile.position.y);
      push_quad(&mut vertices, left, bottom, right, top, color);
    }
    for item in &scene.items {
      let (left, bottom, right, top) = scene_position(scene, item.position.x, item.position.y);
    ...
    for actor in &scene.actors {
      let (left, bottom, right, top) = scene_position(scene, actor.position.x, actor.position.y);
    ...
    for target in &scene.target_positions {
      let (left, bottom, right, top) = scene_position(scene, target.x, target.y);
