//! MCP JSON boundary coverage for Tristar Blaster direct Plasma damage.
...
fn ready_server() -> McpServer {
  ...
  let _ = server.handle_request(
    r#"{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"drl-tristar-test","version":"1"}}}"#,
  );
...
#[test]
fn tristar_blaster_mcp_json_matches_direct_core_typed_damage() {
  let player_position = Position::new(5, 5);
  let target_position = Position::new(6, 5);
  let mut setup =
    ReplayLog::new(46_016, 12, 12, player_position).with_player_config(PlayerSpawnConfig {
      hp: 500,
      max_hp: 500,
      speed: 100,
  ...
  let (initial, _) = ReplayEngine::run(&setup).expect("direct Tristar setup replay");
  let target_id = initial
    .world()
    .actors()
    .values()
    .find(|actor| !actor.is_player())
    .expect("Tristar target")
    .id();
  let command = Command::AttackRanged(target_position);
  let mut direct = initial;
  let expected_events = direct.step(command).expect("direct Tristar shot");
