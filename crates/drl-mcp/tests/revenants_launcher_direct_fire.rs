...
fn ready_server() -> McpServer {
  ...
  let _ = server.handle_request(
    r#"{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"drl-revenants-launcher-test","version":"1"}}}"#,
  );
...
#[test]
fn revenants_launcher_mcp_json_matches_direct_core_typed_damage() {
  let player_position = Position::new(5, 5);
  let target_position = Position::new(6, 5);
  let mut setup =
    ReplayLog::new(46_204, 12, 12, player_position).with_player_config(PlayerSpawnConfig {
      hp: 500,
      max_hp: 500,
      speed: 100,
  ...
  let (initial, _) = ReplayEngine::run(&setup).expect("direct Revenant's Launcher setup replay");
  let target_id = initial
    .world()
    .actors()
    .values()
    .find(|actor| !actor.is_player())
    .expect("Revenant's Launcher target")
    .id();
