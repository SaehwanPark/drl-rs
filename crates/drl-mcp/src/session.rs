...
pub fn game_event_to_json(event: &GameEvent) -> JsonValue {
  ...
  match event {
    ...
    GameEvent::RocketLauncherExplosionScheduled {
      ...
      map.insert(
        "type".to_string(),
        JsonValue::from("RocketLauncherExplosionScheduled"),
      );
      map.insert("entity_id".to_string(), JsonValue::from(entity_id.as_u64()));
      map.insert("target_id".to_string(), JsonValue::from(target_id.as_u64()));
      map.insert("delay".to_string(), JsonValue::from(*delay));
    ...
    GameEvent::RevenantsLauncherExplosionScheduled {
      ...
      map.insert(
        "type".to_string(),
        JsonValue::from("RevenantsLauncherExplosionScheduled"),
      );
      map.insert("entity_id".to_string(), JsonValue::from(entity_id.as_u64()));
      map.insert("target_id".to_string(), JsonValue::from(target_id.as_u64()));
      map.insert("delay".to_string(), JsonValue::from(*delay));
    ...
    GameEvent::MissileLauncherExplosionScheduled {
      ...
      map.insert(
        "type".to_string(),
        JsonValue::from("MissileLauncherExplosionScheduled"),
      );
      map.insert("entity_id".to_string(), JsonValue::from(entity_id.as_u64()));
      map.insert("target_id".to_string(), JsonValue::from(target_id.as_u64()));
      map.insert("delay".to_string(), JsonValue::from(*delay));
...
mod tests {
  ...
  #[test]
  fn rocket_launcher_explosion_schedule_event_projects_to_mcp_json() {
    let value = game_event_to_json(&GameEvent::RocketLauncherExplosionScheduled {
      entity_id: drl_protocol::EntityId::new(1),
      target_id: drl_protocol::EntityId::new(2),
      delay: 40,
      radius: 4,
    ...
    assert_eq!(
      map.get("type").and_then(JsonValue::as_str),
      Some("RocketLauncherExplosionScheduled")
    );
    assert_eq!(map.get("entity_id").and_then(JsonValue::as_i64), Some(1));
    assert_eq!(map.get("target_id").and_then(JsonValue::as_i64), Some(2));
    assert_eq!(map.get("delay").and_then(JsonValue::as_i64), Some(40));
    assert_eq!(map.get("radius").and_then(JsonValue::as_i64), Some(4));
    assert_eq!(map.get("knockback").and_then(JsonValue::as_i64), Some(8));
  }

  #[test]
  fn revenants_launcher_explosion_schedule_event_projects_to_mcp_json() {
    let value = game_event_to_json(&GameEvent::RevenantsLauncherExplosionScheduled {
      entity_id: drl_protocol::EntityId::new(1),
      target_id: drl_protocol::EntityId::new(2),
      delay: 40,
      radius: 3,
    ...
    assert_eq!(
      map.get("type").and_then(JsonValue::as_str),
      Some("RevenantsLauncherExplosionScheduled")
    );
    assert_eq!(map.get("entity_id").and_then(JsonValue::as_i64), Some(1));
    assert_eq!(map.get("target_id").and_then(JsonValue::as_i64), Some(2));
    assert_eq!(map.get("delay").and_then(JsonValue::as_i64), Some(40));
    assert_eq!(map.get("radius").and_then(JsonValue::as_i64), Some(3));
    assert_eq!(map.get("knockback").and_then(JsonValue::as_i64), Some(8));
  }

  #[test]
  fn missile_launcher_explosion_schedule_event_projects_to_mcp_json() {
    let value = game_event_to_json(&GameEvent::MissileLauncherExplosionScheduled {
      entity_id: drl_protocol::EntityId::new(1),
      target_id: drl_protocol::EntityId::new(2),
      delay: 40,
      radius: 3,
    ...
    assert_eq!(
      map.get("type").and_then(JsonValue::as_str),
      Some("MissileLauncherExplosionScheduled")
    );
    assert_eq!(map.get("entity_id").and_then(JsonValue::as_i64), Some(1));
    assert_eq!(map.get("target_id").and_then(JsonValue::as_i64), Some(2));
    assert_eq!(map.get("delay").and_then(JsonValue::as_i64), Some(40));
  ...
  fn test_legal_action_catalog_advertises_chaingun_chainfire_levels() {
    ...
    let eleventh = compute_legal_actions(&session.get_observation().unwrap())
      .into_iter()
      .find(|action| action.action == "Chainfire")
      .expect("eleventh Chaingun chainfire should be advertised");
    assert_eq!(
      eleventh.command,
      Command::AttackRangedChainfire(Position::new(3, 1))
    );
    assert!(eleventh.description.contains("6 projectiles, 6 rounds"));
    session
    ...
    let twelfth = compute_legal_actions(&session.get_observation().unwrap())
      .into_iter()
      .find(|action| action.action == "Chainfire")
      .expect("twelfth Chaingun chainfire should be advertised");
    assert_eq!(
      twelfth.command,
      Command::AttackRangedChainfire(Position::new(3, 1))
    );
    assert!(twelfth.description.contains("6 projectiles, 6 rounds"));
    session
    ...
    let reload = compute_legal_actions(&session.get_observation().unwrap())
      .into_iter()
      .find(|action| action.action == "Reload")
      .expect("Chaingun reload should be advertised before fourteenth chainfire");
    assert_eq!(reload.command, Command::Reload);
    session
      .step(reload.command)
      .expect("reload before fourteenth chainfire action");
    let fourteenth = compute_legal_actions(&session.get_observation().unwrap())
      .into_iter()
      .find(|action| action.action == "Chainfire")
      .expect("fourteenth Chaingun chainfire should be advertised");
    assert_eq!(
      fourteenth.command,
      Command::AttackRangedChainfire(Position::new(3, 1))
    );
    assert!(fourteenth.description.contains("6 projectiles, 6 rounds"));
    session
  ...
  fn chaingun_chainfire_mcp_boundary_matches_direct_core() {
    ...
    let player_config = drl_protocol::PlayerSpawnConfig {
      ...
      max_hp: 50,
      speed: 100,
      initial_items: vec![drl_protocol::ItemSpawnKind::Ammo9mm(80)],
    ...
    let (eleventh_events, eleventh_observation, eleventh_outcome) = session
      .step(eleventh_command)
      .expect("MCP eleventh Chaingun chainfire command");
    ...
    let (twelfth_events, twelfth_observation, twelfth_outcome) = session
      .step(twelfth_command)
      .expect("MCP twelfth Chaingun chainfire command");
    ...
    let (second_reload_events, second_reload_observation, second_reload_outcome) = session
      .step(second_reload_command)
      .expect("MCP second Chaingun chainfire reload");
    ...
    assert!(second_reload_events.iter().any(|event| matches!(
      event,
      GameEvent::WeaponReloaded {
        entity_id,
        ammo_loaded: 36,
    ...
    let (fourteenth_events, fourteenth_observation, fourteenth_outcome) = session
      .step(fourteenth_command)
      .expect("MCP fourteenth Chaingun chainfire command");
