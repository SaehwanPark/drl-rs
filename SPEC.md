...
Current project version: `0.2.239`
...
## 2. Active Implementation Slice: M9 — Frag Shotgun Browser Boundary

### 2.1 Objective

Verify the delivered Frag Shotgun ordinary-fire policy at the direct-core and
BrowserSession boundary. A deterministic one-projectile command must produce
...
- **Observable outcome:** A fixed Frag Shotgun replay setup and one ranged
  ...
  rounds from the 16-round clip and emits one ordered ranged-hit event.
- **Gameplay/replay impact:** Gameplay semantics remain at `53`; replay
  ...
  Project version advances from `0.2.238` to `0.2.239` for the browser-boundary
  verification slice.
...
- **Evidence boundary:** Pinned Frag Shotgun source evidence in
  `docs/legacy-behavior/frag-shotgun-profile.md`, the delivered direct-target
...
changing RNG sampling, or altering gameplay semantics. Frag Shotgun alternate
...
### 2.7di Current Frag Shotgun browser-boundary verification target
