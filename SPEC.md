...
## 2. Active Implementation Slice: M9 — BFG 10K Sixteenth-Level Chainfire
...
fourteenth-, and fifteenth-level chainfire commands with the legacy-pinned
sixteenth warm-up level. After accepted four-, five-, and seven-projectile
bursts leave the weapon at warm-up level fifteen, a valid sixteenth-level command must emit
...
- **Observable outcome:** A sixteenth-level BFG 10K chainfire command is admitted
  only for a living visible target, a loaded clip of at least thirty-five cells,
  and a weapon warm-up level of fifteen. It consumes thirty-five cells, emits
  ...
  slots, and advances warm-up to level sixteen. Ordinary fire still resets the
  warm-up state, and the seventeenth level remains rejected atomically.
- **Gameplay/replay impact:** Gameplay semantics advance from `108` to `109`;
  replay wire/schema, RNG sampling, generator, and ruleset identities remain
  unchanged. Project version advances from `0.2.299` to `0.2.300`.
...
- **Evidence boundary:** Pinned BFG 10K chainfire, shot-cost, and exact-hit
  ...
  remain `NOT_RUN`.
- **Non-goals:** Seventeenth and later chainfire levels, legacy target rotation or
...
thirteenth-, fourteenth-, and fifteenth-level
transitions
already exist.
