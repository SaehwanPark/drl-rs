...
Current project version: `0.2.240`
...
## 2. Active Implementation Slice: M9 — Combat Pistol Browser Boundary

### 2.1 Objective

Verify the delivered Combat Pistol ordinary-fire policy at the direct-core and
...
- **Observable outcome:** A fixed Combat Pistol replay setup and one ranged
  command produce identical direct-core and `BrowserSession` events,
  observations, render effects, and scenes; the direct result consumes one
  round from the 15-round clip and emits one ordered ranged-hit event.
- **Gameplay/replay impact:** Gameplay semantics remain at `53`; replay
  ...
  Project version advances from `0.2.239` to `0.2.240` for the browser-boundary
  verification slice.
...
changing RNG sampling, or altering gameplay semantics. Combat Pistol alternate
...
### 2.7dj Current Combat Pistol browser-boundary verification target
