...
## 2. Active Implementation Slice: M9 — BFG 10K Seventeenth-Level Chainfire
...
fourteenth-, fifteenth-, and sixteenth-level chainfire commands with the
legacy-pinned seventeenth warm-up level. After accepted four-, five-, and
seven-projectile bursts leave the weapon at warm-up level sixteen, a valid
seventeenth-level command must emit seven ordered exact-hit ranged projectiles,
consume thirty-five cells, preserve the existing delayed-explosion metadata,
and advance the warm-up state to level seventeen.
...
- **Observable outcome:** A seventeenth-level BFG 10K chainfire command is admitted
  only for a living visible target, a loaded clip of at least thirty-five cells,
  and a weapon warm-up level of sixteen. It consumes thirty-five cells, emits
  ...
  slots, and advances warm-up to level seventeen. Ordinary fire still resets the
  warm-up state, and the eighteenth level remains rejected atomically.
- **Gameplay/replay impact:** Gameplay semantics advance from `109` to `110`;
  replay wire/schema, RNG sampling, generator, and ruleset identities remain
  unchanged. Project version advances from `0.2.300` to `0.2.301`.
...
- **Evidence boundary:** Pinned BFG 10K chainfire, shot-cost, and exact-hit
  ...
  remain `NOT_RUN`.
- **Non-goals:** Eighteenth and later chainfire levels, legacy target rotation or
...
thirteenth-, fourteenth-, fifteenth-, and sixteenth-level
transitions
already exist.
