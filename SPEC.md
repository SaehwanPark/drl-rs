...
Current project version: `0.2.299`
...
## 2. Active Implementation Slice: M9 — BFG 10K Fifteenth-Level Chainfire
...
fourteenth-level chainfire commands with the legacy-pinned fifteenth warm-up
level. After accepted four-, five-, and seven-projectile bursts leave the weapon
at warm-up level fourteen, a valid fifteenth-level command must emit
...
- **Observable outcome:** A fifteenth-level BFG 10K chainfire command is admitted
  only for a living visible target, a loaded clip of at least thirty-five cells,
  and a weapon warm-up level of fourteen. It consumes thirty-five cells, emits
  ...
  slots, and advances warm-up to level fifteen. Ordinary fire still resets the
  warm-up state, and the sixteenth level remains rejected atomically.
- **Gameplay/replay impact:** Gameplay semantics advance from `107` to `108`;
  replay wire/schema, RNG sampling, generator, and ruleset identities remain
  unchanged. Project version advances from `0.2.298` to `0.2.299`.
...
- **Evidence boundary:** Pinned BFG 10K chainfire, shot-cost, and exact-hit
  ...
  remain `NOT_RUN`.
- **Non-goals:** Sixteenth and later chainfire levels, legacy target rotation or
...
thirteenth-, and fourteenth-level
transitions
already exist.
