...
## 2. Active Implementation Slice: M9/Vertical Fidelity — Shotgun Knockback

### 2.1 Objective

Exercise the existing typed Shotgun knockback and ranged-combat transitions
through a fixed Former Sergeant-profile encounter. The same stable
...
- **Observable outcome:** A declarative encounter, replay execution, and
  browser session agree on the exact arena, configured Shotgun identity, seeded
  ranged hit, Former Sergeant-profile response, one-tile knockback destination,
...
- **Protocol/domain ownership:** `drl-protocol` owns stable item/monster
  identity and commands; core owns combat, knockback, scheduling, and event
  order; render/web own derived observations and effects.
...
- **Non-goals:** New weapon or monster balance equations, broad content
  ...
  runtime Lua, and wall-blocked or multi-tile knockback variants.

### 2.2 Why this slice is bounded

The existing Gate C and Gate D work already centralizes stable item/monster
identity and delivers typed inventory, ranged combat, knockback, scheduling,
and presentation behavior. This slice proves that those pieces survive one
...
### 2.7ab Current vertical Shotgun knockback delivery target
...
encounter extends the chain with a scheduled ranged response. Future composite
