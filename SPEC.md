...
## 2. Active Implementation Slice: M9 — Chaingun Fourteenth-Level Chainfire

### 2.1 Objective

Extend the delivered typed Chaingun first- through thirteenth-level chainfire
commands with one legacy-pinned fourteenth-level continuation. After accepted
three-, four-, and six-projectile bursts leave the weapon at warm-up level
thirteen, a valid fourteenth-level command must emit six ordered ranged
projectiles, consume six rounds, preserve the existing damage and event
ordering, and advance the warm-up state to level fourteen.
...
- **Observable outcome:** A fourteenth-level Chaingun chainfire command is
  admitted only for a living visible target, a loaded clip of at least six
  rounds, and a weapon warm-up level of thirteen. It consumes six rounds, emits
  six ordered ranged outcomes, preserves deterministic post-lethal no-op
  continuation slots, and advances warm-up to level fourteen. Ordinary fire
  still resets the warm-up state, and the fifteenth level remains rejected
  atomically.
- **Gameplay/replay impact:** Gameplay semantics advance from `126` to `127`;
  replay wire/schema, RNG sampling, generator, and ruleset identities remain
  unchanged. Project version advances from `0.2.317` to `0.2.318`.
...
- **Evidence boundary:** Pinned Chaingun profile evidence in
  ...
  remain `NOT_RUN`.
- **Non-goals:** Fifteenth and later chainfire levels, alternate target routing,
...
thirteenth-level transitions already exist. This extension adds only one typed
...
### 2.7ek Historical Nuclear BFG 9000 radius-8 actor-fanout target
...
### 2.7el Historical Standard BFG 9000 ground-item destruction target

The bounded implementation target for this revision extends the delivered
Standard BFG 9000 radius-8 actor fanout with its legacy-pinned ordinary
ground-item destruction rule. Its contract must:

- [x] preserve the existing direct-hit, delay-33/radius-8/knockback-16
  schedule event, clip/action cost, actor fanout, and event ordering;
- [x] resolve stable line-of-sight-cleared radius-8 cells with one `10d6`
  Plasma roll per cell and no distance falloff, retaining source self-safety,
  actor de-duplication, radial integer `damage / 16` knockback, and lethal
  death/drop/game-over follow-up;
- [x] after actor processing on each clear blast cell, remove at most the
  lowest-ID ordinary ground item when that cell's roll is strictly greater
  than `10`; emit `GroundItemDestroyed` before lethal follow-up, and preserve
  higher-ID items and cells whose roll is `10` or less;
- [x] preserve atomic death-drop preflight and rejected-command state identity,
  including ground-item state and RNG, for all newly reachable failures;
- [x] preserve one-roll-per-cell RNG semantics and direct-core, replay,
  ScenarioRunner, MCP, and BrowserSession event/state parity;
- [x] advance project version from `0.2.268` to `0.2.269` and gameplay
  semantics from `77` to `78` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep Nuclear BFG ground-item effects, EFCHAIN secondary explosions,
  higher chainfire levels, scatter/target routing, delayed timing/state-machine
  parity, terrain/content mutation, splash-immunity traits, exact callback
  timing/accuracy, controlled runtime, browser capture, and audiovisual parity
  `NOT_RUN` where comparison evidence is unavailable.

### 2.7em Historical Nuclear BFG 9000 ground-item destruction target

The bounded implementation target for this revision extends the delivered
Nuclear BFG 9000 radius-8 actor fanout with its legacy-pinned ordinary
ground-item destruction rule. Its contract must:

- [x] preserve the existing direct-hit, delay-33/radius-8/knockback-16
  schedule event, clip/action cost, actor fanout, and event ordering;
- [x] resolve stable line-of-sight-cleared radius-8 cells with one `8d6`
  Plasma roll per cell and no distance falloff, retaining source self-safety,
  actor de-duplication, radial integer `damage / 16` knockback, and lethal
  death/drop/game-over follow-up;
- [x] after actor processing on each clear blast cell, remove at most the
...
- [x] preserve one-roll-per-cell RNG semantics and direct-core, replay,
  ScenarioRunner, MCP, and BrowserSession event/state parity;
- [x] advance project version from `0.2.269` to `0.2.270` and gameplay
  semantics from `78` to `79` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep EFCHAIN secondary explosions, higher chainfire levels,
  scatter/target routing, delayed timing/state-machine parity,
  terrain/content mutation, splash-immunity traits, exact callback
  timing/accuracy, controlled runtime, browser capture, and audiovisual parity
  `NOT_RUN` where comparison evidence is unavailable.

### 2.7en Historical BFG 10K second-level chainfire target

The delivered `0.2.271` target extended the BFG 10K first-level chainfire
...
- [x] resolve the legacy level-one formula `shots = 5`, consuming exactly
  twenty-five loaded cells for the five-projectile exact-hit volley;
- [x] preserve each successful hit's delay-25/radius-2/knockback-16 schedule,
  immediate splash resolution, ordered event stream, and deterministic
  post-lethal no-op continuation slots;
...
- [x] advance project version from `0.2.270` to `0.2.271` and gameplay
  semantics from `79` to `80` while preserving replay schema, RNG, generator,
  and ruleset identities;
...
- [x] resolve the legacy level-two formula `shots = 5 + (5 div 2) = 7`,
  consuming exactly thirty-five loaded cells for the seven-projectile exact-hit
  volley;
- [x] preserve each successful hit's delay-25/radius-2/knockback-16 schedule,
  immediate splash resolution, ordered event stream, and deterministic
  post-lethal no-op continuation slots;
- [x] advance warm-up state to level three only after acceptance, preserve it
  across a reload, keep ordinary fire's reset behavior, and reject the fourth
  level without changing game, clip, turn, or RNG state;
- [x] preserve direct-core, reload-backed replay, MCP legal-action/JSON,
  physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.271` to `0.2.272` and gameplay
  semantics from `80` to `81` while preserving replay schema, RNG, generator,
  and ruleset identities;
...
### 2.7er Historical Chaingun second-level chainfire target
...
### 2.7es Historical Chaingun third-level chainfire target

The bounded implementation target for this revision extends the delivered
Chaingun second-level chainfire command with the pinned third warm-up level.
Its contract must:

- [x] preserve the first-level three-projectile/three-round and second-level
  four-projectile/four-round contracts and admit a third command only while the
  weapon warm-up level is two and the target is still a valid visible living
  actor;
- [x] resolve the legacy level-two formula `shots = 4 + (4 div 2) = 6`,
  consuming exactly six loaded 9mm rounds for the six-projectile ranged volley;
- [x] preserve ordered event output and deterministic post-lethal no-op
  continuation slots without changing the existing ordinary-fire behavior;
- [x] advance warm-up state to level three only after acceptance, keep ordinary
  fire's reset behavior, and reject the fourth level without changing game,
...
- [x] preserve direct-core, replay, MCP legal-action/JSON, physical `C` key,
  and BrowserSession event/state parity;
- [x] advance project version from `0.2.275` to `0.2.276` and gameplay
  semantics from `84` to `85` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep fourth-and-later chainfire levels, legacy target rotation,
  scatter/spread routing, exact callback timing/accuracy, controlled runtime,
  browser capture, and audiovisual parity `NOT_RUN` where comparison evidence
...
### 2.7et Historical Minigun second-level chainfire target

The bounded implementation target for this revision extends the delivered
Minigun first-level chainfire command with the pinned second warm-up level.
Its contract must:

- [x] preserve the first-level six-projectile/six-round contract and admit a
  second command only while the weapon warm-up level is one and the target is
  still a valid visible living actor;
- [x] resolve the legacy level-one formula `shots = 8`, consuming exactly
  eight loaded 9mm rounds for the eight-projectile ranged volley;
- [x] preserve ordered event output and deterministic post-lethal no-op
  continuation slots without changing the existing ordinary-fire behavior;
- [x] advance warm-up state to level two only after acceptance, keep ordinary
  fire's reset behavior, and reject the third level without changing game,
...
- [x] preserve direct-core, replay, MCP legal-action/JSON, physical `C` key,
  and BrowserSession event/state parity;
- [x] advance project version from `0.2.276` to `0.2.277` and gameplay
  semantics from `85` to `86` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep third-and-later chainfire levels, legacy target rotation,
  scatter/spread routing, exact callback timing/accuracy, controlled runtime,
  browser capture, and audiovisual parity `NOT_RUN` where comparison evidence
...
### 2.7eu Historical Minigun third-level chainfire target

The delivered `0.2.278` target extended the Minigun second-level chainfire
command with the pinned third warm-up level. Its contract was:

- [x] preserve the first- and second-level six-projectile/six-round and
  eight-projectile/eight-round contracts and admit a third command only while
  the weapon warm-up level is two and the target is still a valid visible
  living actor;
- [x] resolve the legacy level-two formula `shots = 8 + (8 div 2) = 12`,
  consuming exactly twelve loaded 9mm rounds for the twelve-projectile ranged
  volley;
- [x] preserve ordered event output and deterministic post-lethal no-op
  continuation slots without changing the existing ordinary-fire behavior;
- [x] advance warm-up state to level three only after acceptance, keep ordinary
...
- [x] preserve direct-core, replay, MCP legal-action/JSON, physical `C` key,
  and BrowserSession event/state parity;
- [x] advance project version from `0.2.277` to `0.2.278` and gameplay
  semantics from `86` to `87` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep fourth-and-later chainfire levels, legacy target rotation,
  scatter/spread routing, exact callback timing/accuracy, controlled runtime,
  browser capture, and audiovisual parity `NOT_RUN` where comparison evidence
...
### 2.7ex Historical Laser Rifle third-level chainfire target

The delivered `0.2.281` target extended the Laser Rifle first- and second-level
chainfire commands with the pinned third warm-up level. Its contract was:
...
- [x] preserve direct-core, replay, MCP legal-action/JSON, physical `C` key,
  and BrowserSession event/state parity;
- [x] advance project version from `0.2.280` to `0.2.281` and gameplay
...
### 2.7ey Historical Laser Rifle fourth-level chainfire target

The delivered `0.2.282` target extended the Laser Rifle first-, second-, and
third-level chainfire commands with the pinned fourth warm-up level. Its
contract was:

- [x] preserve the first-, second-, and third-level four-, five-, and
  seven-projectile/four-, five-, and seven-cell contracts and admit a fourth
...
- [x] advance warm-up state to level four only after acceptance, keep ordinary
  fire's reset behavior, and reject the fifth level without changing game,
  clip, turn, or RNG state;
- [x] preserve direct-core, replay, MCP legal-action/JSON, physical `C` key,
  and BrowserSession event/state parity;
- [x] advance project version from `0.2.281` to `0.2.282` and gameplay
  semantics from `90` to `91` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep fifth-and-later chainfire levels, legacy target rotation,
  scatter/spread routing, exact timing/accuracy, controlled legacy runtime,
  browser capture, and audiovisual parity `NOT_RUN` where comparison evidence
  is unavailable.

### 2.7ez Historical BFG 10K fourth-level chainfire target

The delivered `0.2.283` target extended the BFG 10K first-, second-, and
third-level chainfire commands with the pinned fourth warm-up level. Its
contract was:

- [x] preserve the first-, second-, and third-level four-, five-, and
  seven-projectile/twenty-, twenty-five-, and thirty-five-cell contracts and
  admit a fourth command only while the weapon warm-up level is three and the
  target is still a valid visible living actor;
- [x] resolve the legacy level-two-and-later formula `shots = 5 + (5 div 2) =
  7`, consuming exactly thirty-five loaded cells for the seven-projectile
  exact-hit volley;
- [x] preserve each successful hit's delay-25/radius-2/knockback-16 schedule,
  immediate splash behavior, ordered event output, and deterministic post-
  lethal no-op continuation slots;
- [x] advance warm-up state to level four only after acceptance, preserve it
  across reloads, keep ordinary fire's reset behavior, and reject the fifth
  level without changing game, clip, turn, or RNG state;
...
- [x] advance project version from `0.2.282` to `0.2.283` and gameplay
  semantics from `91` to `92` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep fifth-and-later chainfire levels, target rotation, scatter/routing,
  delayed timing/state-machine parity, terrain/content effects, splash-immunity
  traits, exact callback timing/accuracy, controlled legacy runtime, browser
  capture, and audiovisual parity `NOT_RUN` where comparison evidence is
  unavailable.

### 2.7f0 Historical Nuclear Plasma fourth-level chainfire target

The delivered `0.2.284` target extended the Nuclear Plasma Rifle first-,
second-, and third-level chainfire commands with the pinned fourth warm-up
level. Its contract was:

- [x] preserve the first-, second-, and third-level four-, six-, and
  nine-projectile/four-, six-, and nine-cell contracts and admit a fourth
  command only while the weapon warm-up level is three and the target is
  still a valid visible living actor;
- [x] resolve the legacy level-two-and-later formula `shots = 6 + (6 div 2) =
  9`, consuming exactly nine loaded cells for the nine-projectile ranged
  volley;
- [x] preserve the existing automatic recharge delay/cadence policy while
  emitting nine ordered ranged events and deterministic post-lethal no-op
  continuation slots;
- [x] advance warm-up state to level four only after acceptance, keep ordinary
  fire's reset behavior, and reject the fifth level or an under-supplied clip
  without changing game, clip, turn, or RNG state;
- [x] preserve direct-core, recharge-backed ScenarioRunner/replay, MCP
  legal-action/JSON, physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.283` to `0.2.284` and gameplay
  semantics from `92` to `93` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep fifth-and-later chainfire levels, overload/map effects, target
  rotation, scatter/routing, exact callback timing/accuracy, controlled legacy
  runtime, browser capture, and audiovisual parity `NOT_RUN` where comparison
  evidence is unavailable.

### 2.7f1 Historical Nuclear Plasma fifth-level chainfire target

The delivered `0.2.285` target extended the Nuclear Plasma Rifle first-,
second-, third-, and fourth-level chainfire commands with the pinned fifth
warm-up level. Its contract was:

- [x] preserve the first-, second-, third-, and fourth-level four-, six-, and
  nine-projectile/nine-cell contracts and admit a fifth command only while the
  weapon warm-up level is four and the target is still a valid visible living
  actor;
- [x] resolve the legacy level-two-and-later formula `shots = 6 + (6 div 2) =
  9`, consuming exactly nine loaded cells for the nine-projectile ranged
  volley;
- [x] preserve the existing automatic recharge delay/cadence policy while
  emitting nine ordered ranged events and deterministic post-lethal no-op
  continuation slots;
- [x] advance warm-up state to level five only after acceptance, keep ordinary
  fire's reset behavior, and reject the sixth level or an under-supplied clip
  without changing game, clip, turn, or RNG state;
- [x] preserve direct-core, recharge-backed ScenarioRunner/replay, MCP
  legal-action/JSON, physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.284` to `0.2.285` and gameplay
  semantics from `93` to `94` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep sixth-and-later chainfire levels, overload/map effects, target
  rotation, scatter/routing, exact callback timing/accuracy, controlled legacy
  runtime, browser capture, and audiovisual parity `NOT_RUN` where comparison
  evidence is unavailable.

### 2.7f2 Historical BFG 10K fifth-level chainfire target

The delivered `0.2.286` target extended the BFG 10K first-, second-, third-,
and fourth-level chainfire commands with the pinned fifth warm-up level. Its
contract was:

- [x] preserve the first-, second-, third-, and fourth-level four-, five-, and
  seven-projectile/twenty-, twenty-five-, and thirty-five-cell contracts and
  admit a fifth command only while the weapon warm-up level is four and the
  target is still a valid visible living actor;
- [x] resolve the legacy level-two-and-later formula `shots = 5 + (5 div 2) =
  7`, consuming exactly thirty-five loaded cells for the seven-projectile
  exact-hit volley;
...
- [x] preserve direct-core, reload-backed ScenarioRunner/replay, MCP
  legal-action/JSON, physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.285` to `0.2.286` and gameplay
  semantics from `94` to `95` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep sixth-and-later chainfire levels, target rotation, scatter/routing,
  delayed timing/state-machine changes, terrain/content effects, splash
  immunity changes, exact callback timing/accuracy, controlled legacy runtime,
  browser capture, and audiovisual parity `NOT_RUN` where comparison evidence
  is unavailable.

### 2.7f3 Historical Laser Rifle fifth-level chainfire target

The delivered `0.2.287` target extended the Laser Rifle first-, second-, third-,
and fourth-level chainfire commands with the pinned fifth warm-up level. Its
contract was:

- [x] preserve the first-, second-, third-, and fourth-level four-, five-, and
  seven-projectile/four-, five-, and seven-cell contracts and admit a fifth
  command only while the weapon warm-up level is four and the target is still
  a valid visible living actor;
- [x] resolve the legacy level-two-and-later formula `shots = 5 + (5 div 2) =
  7`, consuming exactly seven loaded cells for the seven-projectile ranged
  volley;
...
- [x] advance warm-up state to level five only after acceptance, keep ordinary
  fire's reset behavior, and reject the sixth level or an under-supplied clip
  without changing game, clip, turn, or RNG state;
- [x] preserve direct-core, replay, ScenarioRunner, MCP legal-action/JSON,
  physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.286` to `0.2.287` and gameplay
  semantics from `95` to `96` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep sixth-and-later chainfire levels, target rotation, scatter/routing,
  exact timing/accuracy, controlled legacy runtime, browser capture, and
  audiovisual parity `NOT_RUN` where comparison evidence is unavailable.

### 2.7f4 Historical Laser Rifle sixth-level chainfire target

The delivered `0.2.288` target extended the Laser Rifle first-, second-, third-,
fourth-, and fifth-level chainfire commands with the pinned sixth warm-up level.
Its contract was:

- [x] preserve the first- through fifth-level four-, five-, and seven-projectile/
  four-, five-, and seven-cell contracts and admit a sixth command only while
  the weapon warm-up level is five and the target is still a valid visible
  living actor;
- [x] resolve the legacy level-two-and-later formula `shots = 5 + (5 div 2) =
  7`, consuming exactly seven loaded cells for the seven-projectile ranged
  volley;
- [x] preserve ordered event output and deterministic post-lethal no-op
  continuation slots without changing the existing ordinary-fire behavior;
- [x] advance warm-up state to level six only after acceptance, keep ordinary
  fire's reset behavior, and reject the seventh level or an under-supplied clip
  without changing game, clip, turn, or RNG state;
- [x] preserve direct-core, replay, ScenarioRunner, MCP legal-action/JSON,
  physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.287` to `0.2.288` and gameplay
  semantics from `96` to `97` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep seventh-and-later chainfire levels, target rotation, scatter/routing,
  exact timing/accuracy, controlled legacy runtime, browser capture, and
  audiovisual parity `NOT_RUN` where comparison evidence is unavailable.

### 2.7f5 Historical Nuclear Plasma sixth-level chainfire target

The delivered `0.2.289` target extended the Nuclear Plasma Rifle first-,
second-, third-, fourth-, and fifth-level chainfire commands with the pinned
sixth warm-up level. Its contract was:

- [x] preserve the first- through fifth-level four-, six-, and nine-projectile/
  four-, six-, and nine-cell contracts and admit a sixth command only while the
  weapon warm-up level is five and the target is still a valid visible living
  actor;
- [x] resolve the legacy level-two-and-later formula `shots = 6 + (6 div 2) =
  9`, consuming exactly nine loaded cells for the nine-projectile ranged
  volley;
...
- [x] preserve direct-core, recharge-backed ScenarioRunner/replay, MCP
  legal-action/JSON, physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.288` to `0.2.289` and gameplay
  semantics from `97` to `98` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep seventh-and-later chainfire levels, overload/map effects, target
  rotation, scatter/routing, exact callback timing/accuracy, controlled legacy
  runtime, browser capture, and audiovisual parity `NOT_RUN` where comparison
  evidence is unavailable.

### 2.7f6 Historical BFG 10K sixth-level chainfire target

The delivered `0.2.290` target extended the BFG 10K first-, second-, third-,
fourth-, and fifth-level chainfire commands with the pinned sixth warm-up level.
Its contract was:

- [x] preserve the first- through fifth-level four-, five-, and
  seven-projectile/twenty-, twenty-five-, and thirty-five-cell contracts and
  admit a sixth command only while the weapon warm-up level is five and the
  target is still a valid visible living actor;
- [x] resolve the legacy level-two-and-later formula `shots = 5 + (5 div 2) =
  7`, consuming exactly thirty-five loaded cells for the seven-projectile
  exact-hit volley;
...
- [x] preserve direct-core, reload-backed ScenarioRunner/replay, MCP
  legal-action/JSON, physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.289` to `0.2.290` and gameplay
  semantics from `98` to `99` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep seventh-and-later chainfire levels, target rotation, scatter/routing,
  delayed timing/state-machine changes, terrain/content effects, splash
  immunity changes, exact callback timing/accuracy, controlled legacy runtime,
...
### 2.7f7 Historical BFG 10K seventh-level chainfire target

The delivered `0.2.291` target extended the BFG 10K first-, second-, third-,
fourth-, fifth-, and sixth-level chainfire commands with the pinned seventh
warm-up level. Its contract was:

- [x] preserve the first- through sixth-level four-, five-, and
  seven-projectile/twenty-, twenty-five-, and thirty-five-cell contracts and
  admit a seventh command only while the weapon warm-up level is six and the
  target is still a valid visible living actor;
- [x] resolve the legacy level-two-and-later formula `shots = 5 + (5 div 2) =
  7`, consuming exactly thirty-five loaded cells for the seven-projectile
  exact-hit volley;
...
- [x] preserve direct-core, reload-backed ScenarioRunner/replay, MCP
  legal-action/JSON, physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.290` to `0.2.291` and gameplay
  semantics from `99` to `100` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep eighth-and-later chainfire levels, target rotation, scatter/routing,
  delayed timing/state-machine changes, terrain/content effects, splash
  immunity changes, exact callback timing/accuracy, controlled legacy runtime,
  browser capture, and audiovisual parity `NOT_RUN` where comparison evidence
  is unavailable.

### 2.7f8 Historical BFG 10K eighth-level chainfire target

The delivered `0.2.292` target extended the BFG 10K first-, second-, third-,
fourth-, fifth-, sixth-, and seventh-level chainfire commands with the pinned
eighth warm-up level. Its contract was:

- [x] preserve the first- through seventh-level four-, five-, and
  seven-projectile/twenty-, twenty-five-, and thirty-five-cell contracts and
...
- [x] preserve direct-core, reload-backed ScenarioRunner/replay, MCP
  legal-action/JSON, physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.291` to `0.2.292` and gameplay
  semantics from `100` to `101` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep ninth-and-later chainfire levels, target rotation, scatter/routing,
  delayed timing/state-machine changes, terrain/content effects, splash
  immunity changes, exact callback timing/accuracy, controlled legacy runtime,
  browser capture, and audiovisual parity `NOT_RUN` where comparison evidence
  is unavailable.

### 2.7f9 Historical BFG 10K ninth-level chainfire target

The delivered `0.2.293` target extended the BFG 10K first-, second-, third-,
fourth-, fifth-, sixth-, seventh-, and eighth-level chainfire commands with the
pinned ninth warm-up level. Its contract was:

- [x] preserve the first- through eighth-level four-, five-, and
  seven-projectile/twenty-, twenty-five-, and thirty-five-cell contracts and
  admit a ninth command only while the weapon warm-up level is eight and the
  target is still a valid visible living actor;
- [x] resolve the legacy level-two-and-later formula `shots = 5 + (5 div 2) =
  7`, consuming exactly thirty-five loaded cells for the seven-projectile
  exact-hit volley;
- [x] preserve the existing per-hit delayed-explosion schedule and bounded
  radius-2 fanout while emitting seven ordered exact-hit events and
  deterministic post-lethal no-op continuation slots;
- [x] advance warm-up state to level nine only after acceptance, keep ordinary
  fire's reset behavior, and reject the tenth level or an under-supplied clip
  without changing game, clip, turn, or RNG state;
- [x] preserve direct-core, reload-backed ScenarioRunner/replay, MCP
  legal-action/JSON, physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.292` to `0.2.293` and gameplay
  semantics from `101` to `102` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep tenth-and-later chainfire levels, target rotation, scatter/routing,
  delayed timing/state-machine changes, terrain/content effects, splash
  immunity changes, exact callback timing/accuracy, controlled legacy runtime,
  browser capture, and audiovisual parity `NOT_RUN` where comparison evidence
  is unavailable.

### 2.7f10 Historical BFG 10K tenth-level chainfire target

The delivered `0.2.294` target extended the BFG 10K first-, second-, third-,
fourth-, fifth-, sixth-, seventh-, eighth-, and ninth-level chainfire commands
with the pinned tenth warm-up level. Its contract was:

- [x] preserve the first- through ninth-level four-, five-, and
  seven-projectile/twenty-, twenty-five-, and thirty-five-cell contracts and
  admit a tenth command only while the weapon warm-up level is nine and the
  target is still a valid visible living actor;
- [x] resolve the legacy level-two-and-later formula `shots = 5 + (5 div 2) =
  7`, consuming exactly thirty-five loaded cells for the seven-projectile
  exact-hit volley;
- [x] preserve the existing per-hit delayed-explosion schedule and bounded
  radius-2 fanout while emitting seven ordered exact-hit events and
  deterministic post-lethal no-op continuation slots;
- [x] advance warm-up state to level ten only after acceptance, keep ordinary
  fire's reset behavior, and reject the eleventh level or an under-supplied
  clip without changing game, clip, turn, or RNG state;
- [x] preserve direct-core, reload-backed ScenarioRunner/replay, MCP
  legal-action/JSON, physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.293` to `0.2.294` and gameplay
  semantics from `102` to `103` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep eleventh-and-later chainfire levels, target rotation, scatter/routing,
  delayed timing/state-machine changes, terrain/content effects, splash
  immunity changes, exact callback timing/accuracy, controlled legacy runtime,
  browser capture, and audiovisual parity `NOT_RUN` where comparison evidence
  is unavailable.

### 2.7f11 Historical BFG 10K eleventh-level chainfire target

The delivered `0.2.295` target extended the BFG 10K first-, second-, third-,
fourth-, fifth-, sixth-, seventh-, eighth-, ninth-, and tenth-level chainfire
commands with the pinned eleventh warm-up level. Its contract was:

- [x] preserve the first- through tenth-level four-, five-, and
  seven-projectile/twenty-, twenty-five-, and thirty-five-cell contracts and
  admit an eleventh command only while the weapon warm-up level is ten and the
  target is still a valid visible living actor;
- [x] resolve the legacy level-two-and-later formula `shots = 5 + (5 div 2) =
  7`, consuming exactly thirty-five loaded cells for the seven-projectile
  exact-hit volley;
...
- [x] preserve direct-core, reload-backed ScenarioRunner/replay, MCP
  legal-action/JSON, physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.294` to `0.2.295` and gameplay
  semantics from `103` to `104` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep twelfth-and-later chainfire levels, target rotation, scatter/routing,
  delayed timing/state-machine changes, terrain/content effects, splash
  immunity changes, exact callback timing/accuracy, controlled legacy runtime,
  browser capture, and audiovisual parity `NOT_RUN` where comparison evidence
  is unavailable.

### 2.7f12 Historical BFG 10K twelfth-level chainfire target

The delivered `0.2.296` target extended the BFG 10K first- through
eleventh-level chainfire commands with the pinned twelfth warm-up level. Its
contract was:

- [x] preserve the first- through eleventh-level four-, five-, and
  seven-projectile/twenty-, twenty-five-, and thirty-five-cell contracts and
  admit a twelfth command only while the weapon warm-up level is eleven and the
  target is still a valid visible living actor;
- [x] resolve the legacy level-two-and-later formula `shots = 5 + (5 div 2) =
  7`, consuming exactly thirty-five loaded cells for the seven-projectile
  exact-hit volley;
- [x] preserve the existing per-hit delayed-explosion schedule and bounded
  radius-2 fanout while emitting seven ordered exact-hit events and
  deterministic post-lethal no-op continuation slots;
- [x] advance warm-up state to level twelve only after acceptance, keep
  ordinary fire's reset behavior, and reject the thirteenth level or an
  under-supplied clip without changing game, clip, turn, or RNG state;
- [x] preserve direct-core, reload-backed ScenarioRunner/replay, MCP
  legal-action/JSON, physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.295` to `0.2.296` and gameplay
...
### 2.7f14 Historical BFG 10K fourteenth-level chainfire target

The delivered `0.2.298` target extended the BFG 10K first- through
thirteenth-level chainfire commands with the pinned fourteenth warm-up level.
Its contract was:

- [x] preserve the first- through thirteenth-level four-, five-, and
  seven-projectile/twenty-, twenty-five-, and thirty-five-cell contracts and
  admit a fourteenth command only while the weapon warm-up level is thirteen
...
- [x] preserve the existing per-hit delayed-explosion schedule and bounded
  radius-2 fanout while emitting seven ordered exact-hit events and
  deterministic post-lethal no-op continuation slots;
- [x] advance warm-up state to level fourteen only after acceptance, keep
  ordinary fire's reset behavior, and reject the fifteenth level or an
  under-supplied clip without changing game, clip, turn, or RNG state;
- [x] preserve direct-core, reload-backed ScenarioRunner/replay, MCP
  legal-action/JSON, physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.297` to `0.2.298` and gameplay
  semantics from `106` to `107` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep fifteenth-and-later chainfire levels, target rotation,
  scatter/routing, delayed timing/state-machine changes, terrain/content
  effects, splash immunity changes, exact callback timing/accuracy, controlled
  legacy runtime, browser capture, and audiovisual parity `NOT_RUN` where
  comparison evidence is unavailable.

### 2.7f15 Historical BFG 10K fifteenth-level chainfire target

The delivered `0.2.299` target extended the BFG 10K first- through
fourteenth-level chainfire commands with the pinned fifteenth warm-up level.
Its contract was:

- [x] preserve the first- through fourteenth-level four-, five-, and
  seven-projectile/twenty-, twenty-five-, and thirty-five-cell contracts and
  admit a fifteenth command only while the weapon warm-up level is fourteen
  and the target is still a valid visible living actor;
- [x] resolve the legacy level-two-and-later formula `shots = 5 + (5 div 2) =
  7`, consuming exactly thirty-five loaded cells for the seven-projectile
  exact-hit volley;
- [x] preserve the existing per-hit delayed-explosion schedule and bounded
  radius-2 fanout while emitting seven ordered exact-hit events and
  deterministic post-lethal no-op continuation slots;
- [x] advance warm-up state to level fifteen only after acceptance, keep
  ordinary fire's reset behavior, and reject the sixteenth level or an
  under-supplied clip without changing game, clip, turn, or RNG state;
- [x] preserve direct-core, reload-backed ScenarioRunner/replay, MCP
  legal-action/JSON, physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.298` to `0.2.299` and gameplay
  semantics from `107` to `108` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep sixteenth-and-later chainfire levels, target rotation,
  scatter/routing, delayed timing/state-machine changes, terrain/content
  effects, splash immunity changes, exact callback timing/accuracy, controlled
  legacy runtime, browser capture, and audiovisual parity `NOT_RUN` where
  comparison evidence is unavailable.

### 2.7f16 Historical BFG 10K sixteenth-level chainfire target

The delivered `0.2.300` target extended the BFG 10K first- through
fifteenth-level chainfire commands with the pinned sixteenth warm-up level.
Its contract was:

- [x] preserve the first- through fifteenth-level four-, five-, and
  seven-projectile/twenty-, twenty-five-, and thirty-five-cell contracts and
  admit a sixteenth command only while the weapon warm-up level is fifteen
  and the target is still a valid visible living actor;
- [x] resolve the legacy level-two-and-later formula `shots = 5 + (5 div 2) =
  7`, consuming exactly thirty-five loaded cells for the seven-projectile
  exact-hit volley;
- [x] preserve the existing per-hit delayed-explosion schedule and bounded
  radius-2 fanout while emitting seven ordered exact-hit events and
  deterministic post-lethal no-op continuation slots;
- [x] advance warm-up state to level sixteen only after acceptance, keep
  ordinary fire's reset behavior, and reject the seventeenth level or an
  under-supplied clip without changing game, clip, turn, or RNG state;
- [x] preserve direct-core, reload-backed ScenarioRunner/replay, MCP
  legal-action/JSON, physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.299` to `0.2.300` and gameplay
  semantics from `108` to `109` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep seventeenth-and-later chainfire levels, target rotation,
  scatter/routing, delayed timing/state-machine changes, terrain/content
  effects, splash immunity changes, exact callback timing/accuracy, controlled
  legacy runtime, browser capture, and audiovisual parity `NOT_RUN` where
  comparison evidence is unavailable.

### 2.7f17 Historical BFG 10K seventeenth-level chainfire target

The delivered `0.2.301` target extended the BFG 10K first- through
sixteenth-level chainfire commands with the pinned seventeenth warm-up level.
Its contract was:

- [x] preserve the first- through sixteenth-level four-, five-, and
  seven-projectile/twenty-, twenty-five-, and thirty-five-cell contracts and
  admit a seventeenth command only while the weapon warm-up level is sixteen
  and the target is still a valid visible living actor;
- [x] resolve the legacy level-two-and-later formula `shots = 5 + (5 div 2) =
  7`, consuming exactly thirty-five loaded cells for the seven-projectile
  exact-hit volley;
- [x] preserve the existing per-hit delayed-explosion schedule and bounded
  radius-2 fanout while emitting seven ordered exact-hit events and
  deterministic post-lethal no-op continuation slots;
- [x] advance warm-up state to level seventeen only after acceptance, keep
  ordinary fire's reset behavior, and reject the eighteenth level or an
  under-supplied clip without changing game, clip, turn, or RNG state;
- [x] preserve direct-core, reload-backed ScenarioRunner/replay, MCP
  legal-action/JSON, physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.300` to `0.2.301` and gameplay
  semantics from `109` to `110` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep eighteenth-and-later chainfire levels, target rotation,
  scatter/routing, delayed timing/state-machine changes, terrain/content
  effects, splash immunity changes, exact callback timing/accuracy, controlled
  legacy runtime, browser capture, and audiovisual parity `NOT_RUN` where
  comparison evidence is unavailable.

### 2.7f18 Historical BFG 10K eighteenth-level chainfire target

The delivered `0.2.302` target extended the BFG 10K first- through
seventeenth-level chainfire commands with the pinned eighteenth warm-up level.
Its contract was:

- [x] preserve the first- through seventeenth-level four-, five-, and
  seven-projectile/twenty-, twenty-five-, and thirty-five-cell contracts and
  admit an eighteenth command only while the weapon warm-up level is seventeen
  and the target is still a valid visible living actor;
- [x] resolve the legacy level-two-and-later formula `shots = 5 + (5 div 2) =
  7`, consuming exactly thirty-five loaded cells for the seven-projectile
  exact-hit volley;
- [x] preserve the existing per-hit delayed-explosion schedule and bounded
  radius-2 fanout while emitting seven ordered exact-hit events and
  deterministic post-lethal no-op continuation slots;
- [x] advance warm-up state to level eighteen only after acceptance, keep
  ordinary fire's reset behavior, and reject the nineteenth level or an
  under-supplied clip without changing game, clip, turn, or RNG state;
- [x] preserve direct-core, reload-backed ScenarioRunner/replay, MCP
  legal-action/JSON, physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.301` to `0.2.302` and gameplay
  semantics from `110` to `111` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep nineteenth-and-later chainfire levels, target rotation,
  scatter/routing, delayed timing/state-machine changes, terrain/content
  effects, splash immunity changes, exact callback timing/accuracy, controlled
  legacy runtime, browser capture, and audiovisual parity `NOT_RUN` where
  comparison evidence is unavailable.

### 2.7f19 Historical BFG 10K nineteenth-level chainfire target

The delivered `0.2.303` target extended the BFG 10K first- through
eighteenth-level chainfire commands with the pinned nineteenth warm-up level.
Its contract was:

- [x] preserve the first- through eighteenth-level four-, five-, and
  seven-projectile/twenty-, twenty-five-, and thirty-five-cell contracts and
  admit a nineteenth command only while the weapon warm-up level is eighteen
  and the target is still a valid visible living actor;
- [x] resolve the legacy level-two-and-later formula `shots = 5 + (5 div 2) =
  7`, consuming exactly thirty-five loaded cells for the seven-projectile
  exact-hit volley;
- [x] preserve the existing per-hit delayed-explosion schedule and bounded
  radius-2 fanout while emitting seven ordered exact-hit events and
  deterministic post-lethal no-op continuation slots;
- [x] advance warm-up state to level nineteen only after acceptance, keep
  ordinary fire's reset behavior, and reject the twentieth level or an
  under-supplied clip without changing game, clip, turn, or RNG state;
- [x] preserve direct-core, reload-backed ScenarioRunner/replay, MCP
  legal-action/JSON, physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.302` to `0.2.303` and gameplay
  semantics from `111` to `112` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep twentieth-and-later chainfire levels, target rotation,
  scatter/routing, delayed timing/state-machine changes, terrain/content
  effects, splash immunity changes, exact callback timing/accuracy, controlled
  legacy runtime, browser capture, and audiovisual parity `NOT_RUN` where
  comparison evidence is unavailable.

### 2.7f20 Historical BFG 10K twentieth-level chainfire target

The delivered `0.2.304` target extended the BFG 10K first- through
nineteenth-level chainfire commands with the pinned twentieth warm-up level.
Its contract was:

- [x] preserve the first- through nineteenth-level four-, five-, and
  seven-projectile/twenty-, twenty-five-, and thirty-five-cell contracts and
  admit a twentieth command only while the weapon warm-up level is nineteen
  and the target is still a valid visible living actor;
- [x] resolve the legacy level-two-and-later formula `shots = 5 + (5 div 2) =
  7`, consuming exactly thirty-five loaded cells for the seven-projectile
  exact-hit volley;
- [x] preserve the existing per-hit delayed-explosion schedule and bounded
  radius-2 fanout while emitting seven ordered exact-hit events and
  deterministic post-lethal no-op continuation slots;
- [x] advance warm-up state to level twenty only after acceptance, keep
  ordinary fire's reset behavior, and reject the twenty-first level or an
  under-supplied clip without changing game, clip, turn, or RNG state;
- [x] preserve direct-core, reload-backed ScenarioRunner/replay, MCP
  legal-action/JSON, physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.303` to `0.2.304` and gameplay
...
### 2.7f24 Historical Chaingun fourth-level chainfire target
...
### 2.7f25 Historical Chaingun fifth-level chainfire target

The bounded implementation target for this revision extends the Chaingun
first- through fourth-level chainfire commands with the pinned fifth warm-up
level.
Its contract is:

- [x] preserve the first- through fourth-level three-, four-, and six-
  projectile/three-, four-, six-, and six-round contracts and admit a fifth
  command only while the weapon warm-up level is four and the target is still a
  valid visible living actor;
- [x] resolve the legacy level-two-and-later formula `shots = 4 + (4 div 2) =
  6`, consuming exactly six loaded rounds for the ordered ranged volley;
- [x] preserve target, line-of-sight, damage RNG, event ordering, and
...
- [x] preserve direct-core, ScenarioRunner/replay, MCP legal-action/JSON,
  physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.308` to `0.2.309` and gameplay
  semantics from `117` to `118` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep sixth-and-later chainfire levels, target rotation/spread, exact
  callback timing/accuracy, controlled legacy runtime, browser capture, and
  audiovisual parity `NOT_RUN` where comparison evidence is unavailable.

### 2.7f26 Historical Chaingun sixth-level chainfire target

The bounded implementation target for this revision extends the Chaingun
first- through fifth-level chainfire commands with the pinned sixth warm-up
level.
Its contract is:

- [x] preserve the first- through fifth-level three-, four-, and six-
  projectile/three-, four-, six-, six-, and six-round contracts and admit a
  sixth command only while the weapon warm-up level is five and the target is
  still a valid visible living actor;
- [x] resolve the legacy level-two-and-later formula `shots = 4 + (4 div 2) =
  6`, consuming exactly six loaded rounds for the ordered ranged volley;
- [x] preserve target, line-of-sight, damage RNG, event ordering, and
...
- [x] preserve direct-core, ScenarioRunner/replay, MCP legal-action/JSON,
  physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.309` to `0.2.310` and gameplay
  semantics from `118` to `119` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep seventh-and-later chainfire levels, target rotation/spread, exact
  callback timing/accuracy, controlled legacy runtime, browser capture, and
  audiovisual parity `NOT_RUN` where comparison evidence is unavailable.
...
- [x] preserve the first- through sixth-level three-, four-, and six-
  projectile/three-, four-, six-, six-, six-, and six-round contracts and admit
  a seventh command only while the weapon warm-up level is six and the target
  is still a valid visible living actor;
- [x] resolve the legacy level-two-and-later formula `shots = 4 + (4 div 2) =
  6`, consuming exactly six loaded rounds for the ordered ranged volley;
- [x] preserve target, line-of-sight, damage RNG, event ordering, and
...
- [x] preserve direct-core, ScenarioRunner/replay, MCP legal-action/JSON,
  physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.310` to `0.2.311` and gameplay
...
### 2.7f28 Historical Chaingun eighth-level chainfire target

The delivered `0.2.312` target extended the Chaingun first- through
seventh-level chainfire commands with the pinned eighth warm-up level and an
explicit reload-backed continuation boundary.
Its contract is:
...
### 2.7f29 Historical Chaingun ninth-level chainfire target

The delivered `0.2.313` target extended the Chaingun first- through eighth-level
chainfire commands with the pinned ninth warm-up level. Its contract was:

- [x] preserve the first- through eighth-level three-, four-, and six-
  projectile/three-, four-, six-, six-, six-, six-, six-, and six-round
  contracts and admit a ninth command only while the weapon warm-up level is
  eight and the target is still a valid visible living actor;
- [x] resolve the legacy level-two-and-later formula `shots = 4 + (4 div 2) =
  6`, consuming exactly six loaded rounds for the ordered ranged volley after
  the eighth-level continuation;
- [x] preserve target, line-of-sight, damage RNG, event ordering, and
  deterministic post-lethal no-op continuation slots through the existing
  generic ranged path;
- [x] advance warm-up state to level nine only after acceptance, keep ordinary
  fire's reset behavior, and reject the tenth level or an under-supplied clip
  without changing game, clip, turn, and RNG state;
- [x] preserve direct-core, ScenarioRunner/replay, MCP legal-action/JSON,
  physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.312` to `0.2.313` and gameplay
  semantics from `121` to `122` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep tenth-and-later chainfire levels, target rotation/spread, exact
  callback timing/accuracy, controlled legacy runtime, browser capture, and
  audiovisual parity `NOT_RUN` where comparison evidence is unavailable.
...
- [x] preserve the first- through ninth-level three-, four-, and six-
  ...
  contracts and admit a tenth command only while the weapon warm-up level is
  nine and the target is still a valid visible living actor;
- [x] resolve the legacy level-two-and-later formula `shots = 4 + (4 div 2) =
  6`, consuming exactly six loaded rounds for the ordered ranged volley after
  the ninth-level continuation;
- [x] preserve target, line-of-sight, damage RNG, event ordering, and
  deterministic post-lethal no-op continuation slots through the existing
  generic ranged path;
- [x] advance warm-up state to level ten only after acceptance, keep ordinary
  fire's reset behavior, and reject the eleventh level or an under-supplied
  clip without changing game, clip, turn, and RNG state;
- [x] preserve direct-core, ScenarioRunner/replay, MCP legal-action/JSON,
  physical `C` key, and BrowserSession event/state parity;
...
### 2.7f31 Historical Chaingun eleventh-level chainfire target

The delivered `0.2.315` target extended the Chaingun first- through tenth-level
chainfire commands with the pinned eleventh warm-up level. Its contract was:
...
- [x] advance project version from `0.2.314` to `0.2.315` and gameplay
  ...
  and ruleset identities;
- [x] keep twelfth-and-later chainfire levels, target rotation/spread, exact
  callback timing/accuracy, controlled legacy runtime, browser capture, and
  audiovisual parity `NOT_RUN` where comparison evidence is unavailable.

### 2.7f32 Historical Chaingun twelfth-level chainfire target

The delivered `0.2.316` target extended the Chaingun first- through eleventh-level
chainfire commands with the pinned twelfth warm-up level. Its contract was:

- [x] preserve the first- through eleventh-level three-, four-, and six-
  projectile/three-, four-, six-, six-, six-, six-, six-, six-, six-, six-, and
  six-round contracts and admit a twelfth command only while the weapon
  warm-up level is eleven and the target is still a valid visible living actor;
- [x] resolve the legacy level-two-and-later formula `shots = 4 + (4 div 2) =
  6`, consuming exactly six loaded rounds for the ordered ranged volley after
  the eleventh-level continuation;
- [x] preserve target, line-of-sight, damage RNG, event ordering, and
  deterministic post-lethal no-op continuation slots through the existing
  generic ranged path;
- [x] advance warm-up state to level twelve only after acceptance, keep
...
- [x] preserve direct-core, ScenarioRunner/replay, MCP legal-action/JSON,
  physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.315` to `0.2.316` and gameplay
  semantics from `124` to `125` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep thirteenth-and-later chainfire levels, target rotation/spread, exact
  callback timing/accuracy, controlled legacy runtime, browser capture, and
  audiovisual parity `NOT_RUN` where comparison evidence is unavailable.

### 2.7f33 Historical Chaingun thirteenth-level chainfire target

The delivered `0.2.317` target extended the Chaingun first- through twelfth-level
chainfire commands with the pinned thirteenth warm-up level. Its contract was:

- [x] preserve the first- through twelfth-level three-, four-, and six-
  projectile/three-, four-, six-, six-, six-, six-, six-, six-, six-, six-, six-,
  and six-round contracts and admit a thirteenth command only while the weapon
  warm-up level is twelve and the target is still a valid visible living actor;
- [x] resolve the legacy level-two-and-later formula `shots = 4 + (4 div 2) =
  6`, consuming exactly six loaded rounds for the ordered ranged volley after
  the twelfth-level continuation;
- [x] preserve target, line-of-sight, damage RNG, event ordering, and
  deterministic post-lethal no-op continuation slots through the existing
  generic ranged path;
- [x] advance warm-up state to level thirteen only after acceptance, keep
...
- [x] preserve direct-core, ScenarioRunner/replay, MCP legal-action/JSON,
  physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.316` to `0.2.317` and gameplay
  semantics from `125` to `126` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [x] keep fourteenth-and-later chainfire levels, target rotation/spread, exact
  callback timing/accuracy, controlled legacy runtime, browser capture, and
  audiovisual parity `NOT_RUN` where comparison evidence is unavailable.

### 2.7f34 Current Chaingun fourteenth-level chainfire target

The bounded implementation target for this revision extends the Chaingun
first- through thirteenth-level chainfire commands with the pinned fourteenth
warm-up level. Its contract is:

- [x] preserve the first- through thirteenth-level three-, four-, and six-
  projectile/three-, four-, six-, six-, six-, six-, six-, six-, six-, six-, six-,
  and six-round contracts and admit a fourteenth command only while the weapon
  warm-up level is thirteen and the target is still a valid visible living actor;
- [x] resolve the legacy level-two-and-later formula `shots = 4 + (4 div 2) =
  6`, consuming exactly six loaded rounds for the ordered ranged volley after
  the thirteenth-level continuation;
- [x] preserve target, line-of-sight, damage RNG, event ordering, and
  deterministic post-lethal no-op continuation slots through the existing
  generic ranged path;
- [x] advance warm-up state to level fourteen only after acceptance, keep
...
- [x] preserve direct-core, ScenarioRunner/replay, MCP legal-action/JSON,
  physical `C` key, and BrowserSession event/state parity;
- [x] advance project version from `0.2.317` to `0.2.318` and gameplay
  semantics from `126` to `127` while preserving replay schema, RNG, generator,
  and ruleset identities;
- [ ] keep fifteenth-and-later chainfire levels, target rotation/spread, exact
  callback timing/accuracy, controlled legacy runtime, browser capture, and
  audiovisual parity `NOT_RUN` where comparison evidence is unavailable.
