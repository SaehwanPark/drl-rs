...
Current project version: `0.2.236`
...
## 2. Active Implementation Slice: M9 — Super Shotgun Browser Boundary

### 2.1 Objective

Verify the delivered Super Shotgun ordinary-fire policy at the direct-core and
BrowserSession boundary. A deterministic two-projectile command must produce
...
- **Observable outcome:** A fixed Super Shotgun replay setup and one ranged
  command produce identical direct-core and `BrowserSession` events,
  observations, render effects, and scenes; the direct result consumes two
  shells from the two-shell clip and emits two ordered ranged-hit events.
- **Gameplay/replay impact:** Gameplay semantics remain at `53`; replay
  ...
  Project version advances from `0.2.235` to `0.2.236` for the browser-boundary
  verification slice.
...
- **Evidence boundary:** Pinned Super Shotgun source evidence in
  `docs/legacy-behavior/super-shotgun-profile.md`, the delivered direct-target
...
changing RNG sampling, or altering gameplay semantics. Super Shotgun alternate
...
### 2.7df Current Super Shotgun browser-boundary verification target
