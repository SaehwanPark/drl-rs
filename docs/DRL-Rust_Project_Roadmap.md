...
## 3. Current Progress Summary (`VERSION` 0.2.90)
...
- **M1/M2 Correctness Slice (`0.2.89`–`0.2.90`)**: Ranged-command target
  legality and range are prepared before ammo/RNG mutation; equip commands
  validate item existence and slot eligibility before inventory mutation.
  Out-of-range, blocked-line, and non-equippable-item rejections are covered by
  exact `Game` equality tests. Other command-family rejection paths remain
  active Gate A follow-up work.
