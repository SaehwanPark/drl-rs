---
title: "Weapons & Items Catalog"
description: "Comprehensive armory encyclopedia: ballistic firearms, energy weapons, mod packs, armors, and unique exotic items."
---

# Weapons & Items Catalog

**drl-rs** features an extensive arsenal of firearms, heavy ordnance, energy weapons, defensive combat armor, modification packs, and powerful artifact equipment.

---

## 🔫 Standard Firearm Armory

### 1. Pistols & Sidearms
- **Pistol**: Standard-issue 9mm sidearm. Holds 12 rounds. Fast reload (single-turn magazine swap). Supports **Aimed Fire** (+3 accuracy at double action cost).
- **Combat Pistol**: Enhanced tactical sidearm with upgraded base accuracy and rapid chambering.

### 2. Shotguns & Scatterguns
- **Standard Shotgun**: Pump-action 12-gauge scattergun. Holds 1 shell. Fires 7 pellets with 1-tile knockback displacement.
- **Combat Shotgun**: Magazine-fed tactical shotgun with a 5-shell capacity. Supports single-shell tactical loading and chamber cycling.
- **Double Shotgun**: Break-action twin-barrel shotgun. Discharges 2 shells simultaneously for 14 high-impact pellets and heavy knockback.
- **Super Shotgun**: Sawn-off twin-barrel monster. Fires devastating dual-shell volleys at close range.
- **Assault Shotgun**: Full-auto drum-fed combat shotgun with rapid clearing capabilities.

### 3. Rapid-Fire Ballistic Weapons
- **Chaingun**: 4-round burst 9mm rotary weapon. Excellent mid-range suppression against swarms. Supports a first-level 3-projectile chainfire burst.
- **Minigun**: Heavy 8-barrel rotary machine gun. Chews through ammo at 8 rounds per salvo, with first-level 6-projectile, second-level 8-projectile, and third-level 12-projectile chainfire bursts for open combat corridors.

### 4. Heavy Rocket & Explosive Launchers
- **Rocket Launcher**: High-explosive ordnance firing single rockets with a typed direct Fire hit and bounded radius-4 Fire splash; Red Armor's typed 25% Fire resistance applies to direct and splash damage, while post-falloff damage above 10 destroys one lowest-ID ordinary ground item per blast cell.
- **Missile Launcher**: Multi-tube launcher holding up to 4 rockets for continuous area-of-effect suppression. Its direct target hit and bounded center-inclusive radius-3 fanout use typed Fire (so Red Armor's 25% Fire resistance applies before flat protection); each clear blast cell rolls `6d6`, applies integer distance falloff and radial `damage / 8` knockback, damages the firing actor, and destroys one lowest-ID ordinary ground item when post-falloff damage exceeds 10. A resolved miss still emits the explosion schedule/fanout, while single-rocket tactical reload remains available.
- **Revenant's Launcher**: Unique single-rocket launcher with an exact-hit policy, typed direct Fire damage, and a bounded center-inclusive radius-3 Fire splash (so Red Armor's 25% Fire resistance applies before flat protection on both paths); splash uses legacy distance falloff, radial knockback, and thresholded ordinary ground-item destruction. It retains single-rocket tactical reload.
- **Anti-Freak Jackal**: Unique aimed-fire sidearm whose successful direct hit
  and radius-1 `5d3` splash use typed `Fire` damage; Red Armor's typed 25% Fire
  resistance applies before its flat protection on both paths.

---

## ⚡ Energy & Plasma Weaponry

- **Blaster**: Self-charging micro-cell energy pistol with typed direct Plasma damage (so Blue Armor's 20% Plasma resistance applies before flat protection). Holds 10 energy charges; automatically regenerates charges over time. Supports Aimed Fire.
- **Plasma Shotgun**: Exotic one-projectile, three-cell shotgun whose direct `7d3` hit uses typed Plasma mitigation, so Blue Armor's 20% Plasma resistance applies before flat protection; spread, falloff, and knockback remain separate policies.
- **Tristar Blaster**: Exotic three-projectile, fifteen-cell volley whose successful direct target hits use typed Plasma mitigation, so Blue Armor's 20% Plasma resistance applies before flat protection; spread, routing, delayed effects, and knockback remain separate policies.
- **Laser Rifle**: Long-range beam projector firing a 5-cell precision coherent light volley with typed direct Plasma damage (so Blue Armor's 20% Plasma resistance applies before flat protection), first-level 4-projectile, second-level 5-projectile, and third-, fourth-, fifth-, sixth-, and seventh-level 7-projectile chainfire bursts.
- **Plasma Rifle**: Rapid-fire energy rifle discharging 6-cell plasma bolts with typed direct Plasma damage (so Blue Armor's 20% Plasma resistance applies before flat protection), thermal splash damage, and first-level 4-projectile plus second-level 6-projectile chainfire bursts.
- **Nuclear Plasma Rifle**: Experimental nuclear core plasma rifle with typed direct Plasma damage (so Blue Armor's 20% Plasma resistance applies before flat protection). It has a 24-cell clip, first-, second-, third-, fourth-, fifth-, sixth-, and seventh-level 4/6/9/9/9/9/9-projectile chainfire (four, six, nine, nine, nine, nine, and nine cells), and continuous internal power recharge.
- **BFG 9000**: The ultimate bio-force weapon. Fires a massive 40-cell typed Plasma ball with a typed direct hit and radius-8 splash, plus massive 16-tile kinetic knockback.
- **Nuclear BFG 9000**: Self-charging exact-hit variant consuming 40 cells per shot. Its `8d6 DAMAGE_SPLASMA` direct target hit and radius-8 fanout use typed Plasma mitigation; recharge and alternate-overload behavior remain separate policies.
- **BFG 10K**: Rapid-cycling experimental variant firing five 5-cell exact-hit plasma charges with typed direct Plasma mitigation and delayed schedule explosions; its first-, second-, third-, fourth-, fifth-, sixth-, seventh-, eighth-, ninth-, tenth-, eleventh-, twelfth-, thirteenth-, fourteenth-, fifteenth-, sixteenth-, seventeenth-, eighteenth-, nineteenth-, twentieth-, and twenty-first-level chainfire emits four, five, seven, seven, seven, seven, seven, seven, seven, seven, seven, seven, seven, seven, seven, seven, seven, seven, seven, seven, and seven exact-hit charges for 20, 25, 35, 35, 35, 35, 35, 35, 35, 35, 35, 35, 35, 35, 35, 35, 35, 35, 35, 35, and 35 cells.

---

## 🛡️ Armor & Protective Suits

| Armor | Protection | Special Mechanics & Behavior |
|---|---|---|
| **Green Armor** | 30% Damage Reduction | Standard combat security vest. Durable and lightweight. |
| **Blue Armor** | 2 flat protection; 20% Plasma resistance on typed Blaster/Plasma Shotgun/Tristar Blaster/Laser Rifle/Plasma Rifle/Nuclear Plasma Rifle/BFG 10K/Standard/Nuclear BFG direct/fanout and Null Pointer fanouts; Null Pointer fanouts apply one-third flat protection | Heavy kevlar and ceramic plate suit. Broader resistance aggregation remains open. |
| **Red Armor** | 4 flat protection; 25% Fire resistance on typed Rocket direct/splash, Anti-Freak direct/splash, Missile Launcher direct/splash, and Revenant's Launcher direct/splash | High-grade armor that reduces typed Fire damage before flat protection. Broader resistance aggregation remains open. |
| **Lava Armor** | 75% Thermal Reduction | Self-repairing suit. Standing in lava tiles recharges durability over time. |
| **Malek's Armor** | 60% All-round Reduction | Ancient demonic power armor. Slowly regenerates durability over time. |
| **Medical Powerarmor** | 50% All-round Reduction | Equipped with automated trauma systems that stabilize player HP when critical. |

---

## 🔮 Exotic & Unique Artifacts

- **Trigun**: Legendary triple-barrel handgun. Supports **Alt-Reload** for a high-risk, catastrophic level-clearing energy detonation.
- **Subtle Knife**: Interdimensional blade. Can be **Invoked** to cut through space and instantly sever adjacent enemy lifelines.
- **Grammaton Beretta**: Dual-action cleric firearm supporting Single, 3-round Burst, and Full-Auto firing modes.
- **Acid Spitter**: Bio-organic corrosive projector. Uses environmental acid pools to reload corrosive bio-rounds.
- **Null Pointer**: Experimental glitch weapon. Applies a fixed `10d1` typed Plasma splash to visible actors; Blue Armor's catalog-defined 20% Plasma resistance applies before one-third of its flat protection for this SPLASMA-style fanout.
- **Phase Device**: Emergency teleportation device. Instantly relocates the user to a clear floor tile away from danger.
- **Large / Small Medpacks**: Field medical kits restoring health up to the maximum hit point threshold.

---

## 🔧 Modification Packs

Mod packs allow deep customization of weapon and armor attributes:
- **Power Mod (P)**: Increases base damage output and projectile damage dice.
- **Agility Mod (A)**: Reduces firing and movement action energy costs.
- **Bulk Mod (B)**: Expands magazine and ammunition clip capacity.
- **Technical Mod (T)**: Improves accuracy, reduces reload time, or unlocks weapon fire modes.
