...
eleventh-level chainfire for `0.2.315`;
twelfth-and-later chainfire levels,
...
- `src/dfbeing.pas:1477-1491` resolves ordinary fire to four projectiles,
  ...
  (`4 + (4 div 2)`); this slice bounds the eleventh level while leaving later
...
and eleventh-level chainfire fragments. The existing
...
rounds at level one, and six rounds at levels two through ten atomically, emits
...
Twelfth-and-later chainfire levels, legacy target rotation/spread, exact
