...
Status: delivered typed pump/reload behavior profile through `0.2.215`;
...
under-supplied clips reject atomically. Earlier transition deliveries advanced
gameplay semantics from `20` to `21`; the `0.2.212` reload profile and `0.2.215`
pump-action extension keep current semantics `41` unchanged. Ammo-pack
behavior, partial-reserve policy, exact legacy timing, controlled runtime
comparison, and chamber presentation/audio remain `NOT_RUN`.
