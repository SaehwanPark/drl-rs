...
## Historical DRL-Rust boundary

The earlier gameplay-semantics `34` delivery (project version `0.2.187`)
extended the existing typed exact-hit policy to Revenant’s Launcher. Valid
visible, in-range shots bypassed only the to-hit RNG while retaining
clip/action-cost checks, damage RNG, and existing attack/damage events;
invalid commands remained atomic. The profile-only `0.2.213` revision keeps
current gameplay semantics `41` unchanged. Homing, projectile routing, delayed
explosions, mods, controlled legacy runtime, and audio/visual comparison are
`NOT_RUN` or separate future slices.
