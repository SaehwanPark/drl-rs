...
- `src/dfbeing.pas:1486-1493` applies the chainfire adjustment by warm-up
  ...
  BFG 10K's five configured shots, the first three levels therefore emit four,
  five, and seven projectiles; `src/dfbeing.pas:1496-1515` charges the
...
Gameplay semantics `81` (project version `0.2.272`) extends the typed BFG 10K
chainfire command with the pinned third warm-up level. At level zero, a valid
...
level two accepts a valid visible target and at least thirty-five loaded cells,
produces seven ordered exact-hit outcomes, consumes thirty-five cells,
preserves the existing per-hit `Bfg10kExplosionScheduled` metadata, and
advances warm-up to level three. Post-lethal slots remain deterministic no-op
misses, ordinary fire resets the warm-up state, and level three requests remain
atomic rejections. Direct-core, reload-backed replay, MCP legal-action/JSON,
physical browser key, and BrowserSession parity plus atomic under-supply
rejection are covered. Fourth and later levels, scatter/target rotation,
projectile routing, delayed explosion
