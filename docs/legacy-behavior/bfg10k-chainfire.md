...
- `src/dfbeing.pas:1486-1493` applies the chainfire adjustment by warm-up
  ...
  BFG 10K's five configured shots, the first six bounded levels therefore
  emit four, five, seven, seven, seven, and seven projectiles;
...
Gameplay semantics `99` (project version `0.2.290`) extends the typed BFG 10K
chainfire command with the pinned sixth warm-up level. At level zero, a valid
...
levels two through five accept a valid visible target and at least thirty-five
...
metadata, and advance warm-up through levels three through six. Post-lethal
slots remain deterministic no-op misses, ordinary fire resets the warm-up
state, and seventh-level and later requests remain atomic rejections.
Direct-core,
reload-backed replay, ScenarioRunner, MCP legal-action/JSON, physical browser
key, and BrowserSession parity plus atomic seventh-level and under-supply
rejection are covered. Seventh and later levels, scatter/target rotation,
