...
- `src/dfbeing.pas:1486-1493` applies the chainfire adjustment by warm-up
  ...
  BFG 10K's five configured shots, the first ten bounded levels therefore
  emit four, five, seven, seven, seven, seven, seven, seven, seven, and seven projectiles;
...
Gameplay semantics `103` (project version `0.2.294`) extends the typed BFG 10K
chainfire command with the pinned tenth warm-up level. At level zero, a valid
...
levels two through nine accept a valid visible target and at least thirty-five
...
metadata, and advance warm-up through levels three through ten. Post-lethal
slots remain deterministic no-op misses, ordinary fire resets the warm-up
state, and eleventh-level and later requests remain atomic rejections.
Direct-core, reload-backed replay, ScenarioRunner, MCP legal-action/JSON,
physical browser key, and BrowserSession parity plus atomic tenth-level and under-supply
rejection are covered. Eleventh and later levels, scatter/target rotation,
