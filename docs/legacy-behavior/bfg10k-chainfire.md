...
- `src/dfbeing.pas:1486-1493` applies the chainfire adjustment by warm-up
  ...
  BFG 10K's five configured shots, the first eighteen bounded levels therefore
  emit four, five, and sixteen seven-projectile bursts;
...
Gameplay semantics `111` (project version `0.2.302`) extends the typed BFG 10K
chainfire command with the pinned eighteenth warm-up level. At level zero, a valid
...
levels two through seventeen accept a valid visible target and at least thirty-five
...
metadata, and advance warm-up through levels three through eighteen. Post-lethal
slots remain deterministic no-op misses, ordinary fire resets the warm-up
state, and nineteenth-level and later requests remain atomic rejections.
Direct-core, reload-backed replay, ScenarioRunner, MCP legal-action/JSON,
physical browser key, and BrowserSession parity plus atomic eighteenth-level and under-supply
rejection are covered. Nineteenth and later levels, scatter/target rotation,
