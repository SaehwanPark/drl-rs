...
- `src/dfbeing.pas:1486-1493` applies the chainfire adjustment by warm-up
  ...
  BFG 10K's five configured shots, the first five bounded levels therefore
  emit four, five, seven, seven, and seven projectiles;
...
Gameplay semantics `95` (project version `0.2.286`) extends the typed BFG 10K
chainfire command with the pinned fifth warm-up level. At level zero, a valid
...
levels two through four accept a valid visible target and at least thirty-five
...
metadata, and advance warm-up through levels three through five. Post-lethal
slots remain deterministic no-op misses, ordinary fire resets the warm-up
state, and level five requests remain atomic rejections. Direct-core,
reload-backed replay, ScenarioRunner, MCP legal-action/JSON, physical browser
key, and BrowserSession parity plus atomic under-supply rejection are covered.
Sixth and later levels, scatter/target rotation, projectile routing, delayed
explosion damage/geometry/knockback, controlled legacy runtime, browser capture,
and audiovisual comparison remain `NOT_RUN` or open.
