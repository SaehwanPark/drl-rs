...
- `src/dfbeing.pas:1486-1493` applies the chainfire adjustment by warm-up
  ...
  BFG 10K's five configured shots, the first sixteen bounded levels therefore
  emit four, five, and fourteen seven-projectile bursts;
...
Gameplay semantics `109` (project version `0.2.300`) extends the typed BFG 10K
chainfire command with the pinned sixteenth warm-up level. At level zero, a valid
...
levels two through fifteen accept a valid visible target and at least thirty-five
...
metadata, and advance warm-up through levels three through sixteen. Post-lethal
slots remain deterministic no-op misses, ordinary fire resets the warm-up
state, and seventeenth-level and later requests remain atomic rejections.
Direct-core, reload-backed replay, ScenarioRunner, MCP legal-action/JSON,
physical browser key, and BrowserSession parity plus atomic sixteenth-level and under-supply
rejection are covered. Seventeenth and later levels, scatter/target rotation,
