...
- `src/dfbeing.pas:1486-1493` applies the chainfire adjustment by warm-up
  ...
  BFG 10K's five configured shots, the first two levels therefore emit four
  and five projectiles; `src/dfbeing.pas:1496-1515` charges the resulting
  projectile count through the existing shot-cost helper.
...
Gameplay semantics `80` (project version `0.2.271`) extends the typed BFG 10K
chainfire command with the pinned second warm-up level. At level zero, a valid
...
consume twenty-five cells, preserve the existing per-hit
`Bfg10kExplosionScheduled` metadata, and advance warm-up to level two.
Post-lethal slots remain deterministic no-op misses, ordinary fire resets the
warm-up state, and level two requests remain atomic rejections. Direct-core,
replay, MCP legal-action/JSON, physical browser key, and BrowserSession parity
plus atomic under-supply rejection are covered. Third and later levels,
