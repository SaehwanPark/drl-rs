...
- `src/dfbeing.pas:1486-1493` applies the chainfire adjustment by warm-up
  ...
  BFG 10K's five configured shots, the first seventeen bounded levels therefore
  emit four, five, and fifteen seven-projectile bursts;
...
Gameplay semantics `110` (project version `0.2.301`) extends the typed BFG 10K
chainfire command with the pinned seventeenth warm-up level. At level zero, a valid
...
levels two through sixteen accept a valid visible target and at least thirty-five
...
metadata, and advance warm-up through levels three through seventeen. Post-lethal
slots remain deterministic no-op misses, ordinary fire resets the warm-up
state, and eighteenth-level and later requests remain atomic rejections.
Direct-core, reload-backed replay, ScenarioRunner, MCP legal-action/JSON,
physical browser key, and BrowserSession parity plus atomic seventeenth-level and under-supply
rejection are covered. Eighteenth and later levels, scatter/target rotation,
