...
| Web/UI policy | `crates/drl-web/src/dom.rs` (`inventory_markup`, `escape_html`), `crates/drl-web/src/input.rs` | Inventory markup, browser actions, and item-facing controls | Explicit UI behavior and escaping tests; not an identity registry. |
