...
## 3. Current Progress Summary (`VERSION` 0.2.358)
...
- **M9 Standard BFG 9000 direct Plasma classification (`0.2.334`, delivered in
  PR #450):** The direct Standard BFG 9000 target damage now uses the existing
  ...
  already delivered radius-8 splash/item effects, and every other direct
...
- **M9 BFG 10K direct Plasma classification (`0.2.336`, delivered in PR #452):**
  Each successful five-projectile BFG 10K direct target hit now uses typed
  Plasma mitigation so Blue Armor resistance applies before flat protection.
  Existing chainfire, radius-2 splash, ground-item, schedule, ordering,
  raw-damage, and RNG behavior remain unchanged. Independent review and
  hosted Repository/WASM checks pass; the documented solo-maintainer
  Review-policy exception remains in force.
- **M9 Plasma Rifle direct Plasma classification (`0.2.337`, delivered in PR
  #453):** Each successful ordinary-fire and already implemented
  first/second-level chainfire direct target hit now uses typed Plasma
  mitigation so Blue Armor resistance applies before flat protection. The
  six-projectile volley, chainfire clip costs, warm-up state, raw rolls, RNG
  order, event ordering, replay/MCP/browser projections, and rejection
...
- **M9 Blaster direct Plasma classification (`0.2.340`, delivered in PR
  #456):** Each successful ordinary and aimed Blaster direct target hit now
  uses typed Plasma mitigation so Blue Armor's catalog-defined 20% resistance
  applies before flat protection. The one-projectile shot, one-cell cost,
  recharge timer, no-reload policy, raw damage, RNG order, event ordering,
  replay/MCP/browser projections, and rejection boundaries remain unchanged.
...
- **Native frontend boundary (`0.2.343`, delivered in PR #460):** The branch
  ...
  candidate `058011a` passed the independent correction-range review; merge
...
- **Delivered M9 Mega Buster exact dice and Fire/Acid radius-one fanout (`0.2.358`):**
  Each Mega Buster profile now consumes one deterministic RNG sample per die
  (`1d8`, `4d2`, `4d2`, `1d10`), matching its legacy dice distribution while
  preserving bounded inclusive damage ranges and typed Physical, Fire, Acid, and
  Plasma mitigation. Fire and Acid modes emit a typed delay-40/radius-one/
  knockback-8 schedule event and immediately resolve the shared center-first
  radius-one environmental splash helper on both hits and misses, rolling
  independent `4d2` damage per clear blast cell. Ordinary fire retains its
  three-projectile early-stop and nine-ammo contract without same-volley morph
  mutation, and all radius-one death-drop destinations are preflighted before
  clip/RNG mutation. Replay gameplay semantics advance from `151` to `152`.
  Focused core, MCP, and BrowserSession parity tests pass; legacy traversal
  metrics, delayed queue, same-volley mutation, controlled legacy runtime,
  audiovisual, and human acceptance remain open or `NOT_RUN`.
...
- **M9 next bounded candidate:** The next unresolved heavy-weapon or exotic
  branch should be selected only after the remaining stop gates and evidence
  boundaries are re-audited.
...
- **Delivered Slice — M9 Blaster direct Plasma classification (`0.2.340`):**
  ...
  ordinary and aimed Blaster direct target hits through typed Plasma mitigation,
  so Blue Armor's catalog-defined 20% resistance applies before flat
  protection. The one-projectile shot, one-cell cost, recharge timer,
  no-reload policy, raw damage, RNG/replay identities, event ordering,
  ...
  exception. Recharge callback timing/state, spread/routing, SPLASMA divisors,
...
- **Delivered Slice — M9 Plasma Rifle direct Plasma classification
  ...
  target hits through typed Plasma mitigation, so Blue Armor's catalog-defined
  20% resistance applies before flat protection. Raw damage, six-projectile
  volley and chainfire clip costs, warm-up state, RNG/replay identities, event
  ordering, rejection boundaries, and boundary projections remain unchanged.
  The independent determinism review returned PASS; local and hosted
  Repository/WASM checks pass under the documented solo-maintainer
  Review-policy exception. Higher chainfire levels, overcharge, spread/routing,
...
- **Delivered Slice — M9 BFG 10K direct Plasma classification (`0.2.336`):**
  ...
  successful BFG 10K direct target hit through typed Plasma mitigation, so Blue
  Armor's catalog-defined 20% resistance applies before flat protection. Raw
  damage, five-projectile cost/order, chainfire, radius-2 splash, ground-item
  behavior, schedules, rejection boundaries, and boundary projections remain
  unchanged. The independent determinism review returned PASS; local and
  hosted Repository/WASM checks pass under the documented solo-maintainer
  Review-policy exception. Scatter/projectile routing, delayed timing,
  SPLASMA divisors, broader resistance aggregation, and other legacy/runtime
...
- **Delivered Slice — M9 Standard BFG 9000 direct Plasma classification
  ...
  routes only the Standard BFG 9000 direct target damage through typed Plasma
  mitigation. Blue Armor's catalog-defined 20% resistance applies before flat
  protection; raw damage, exact-hit behavior, RNG state, splash/item behavior,
  rejection boundaries, and boundary projections remain unchanged. The
  independent `/root/rocket_review` determinism review returned PASS; local
  and hosted Repository/WASM checks pass under the documented solo-maintainer
  Review-policy exception. Non-Rocket direct Plasma, SPLASMA divisors,
  broader resistance aggregation, and other legacy/runtime surfaces remain
  open or `NOT_RUN`.
...
- [x] Standard BFG 9000 direct target damage is classified as typed `Plasma`, so
  Blue Armor's catalog-defined 20% resistance applies before flat protection;
  raw damage, exact-hit behavior, RNG order, splash/item effects, and other
  direct weapons remain unchanged. PR #450 merged as `9132d82` after focused,
...
- [x] BFG 10K direct-target volley damage is classified as typed `Plasma`, so
  ...
  direct/replay tests, independent review, and hosted Repository/WASM evidence
  ...
  `NOT_RUN`.
- [x] Plasma Rifle ordinary-fire and first/second-level chainfire direct target
  ...
  replay tests, independent review, and hosted Repository/WASM evidence pass
...
- [x] Laser Rifle ordinary-fire and first-through-seventh-level chainfire
  ...
  audiovisual parity remain open or `NOT_RUN`.
- [x] Blaster ordinary and aimed direct target damage is classified as typed
  ...
  RNG order, event ordering, rejection boundaries, and MCP/BrowserSession
  projections remain unchanged. Focused direct/aimed/replay tests,
  independent review, and hosted Repository/WASM evidence pass in merged PR
...
- [x] Define the native frontend boundary over `Command`, fair observations,
  ...
  plan (`0.2.343`, delivered in PR #460 as `ee38357`; implementation
  candidate `058011a`).
- [x] Add the minimal `drl-desktop` `winit`/`wgpu` window, keyboard, physical
  ...
  validator (`0.2.343`, delivered in PR #460 as `ee38357`).
