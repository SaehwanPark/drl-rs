...
- `crates/drl-render`: pure scene construction, deterministic pixel viewport
  ...
  mappings, outline-mask transport, renderer-neutral animation metadata, and
  progress-selected frame UVs.
...
- `crates/drl-web`: browser session, validated same-origin texture-source
  ...
  progress-selected frame UVs, and legacy alpha cutoff, Winit/WebGPU scene
