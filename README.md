...
- Versioned delivery:
  - `VERSION` is the canonical `x.y.z` project value (currently `0.2.244`),
  ...
  - The typed behavior vocabulary includes immutable Acid Spitter, Grammaton,
    ...
    Super Shotgun, Minigun, Chaingun, Laser Rifle, Assault Shotgun,
    Revenant's Launcher,
    ...
    five-projectile ordinary fire consumes five cells; Combat Shotgun's
    ...
    consumes one 9mm round, while its typed aimed fire adds +3 accuracy and
    doubles action cost; Rocket Launcher's typed ordinary fire resolves one
...
- Staged work:
  ...
  - M9 now includes definition-backed rocket and power-cell ammo families,
    ...
    Trigun's confirmed alternate reload/nuke transition, Acid Spitter's
    immutable terrain-reload profile and single-round Acid-to-Water transition,
    Grammaton's typed single/burst/auto fire-mode cycle and immutable mode
    profile,
    ...
    the Double Shotgun clip-reload encounter's dual-shot fire, blocked
    knockback, two-projectile resolution, shell consumption, and presentation effects
    have the same vertical scenario/replay/browser-boundary coverage; its
    immutable `DOUBLE_SHOTGUN_BEHAVIOR` profile records the dual-shot count and
    shell cost;
