...
- Versioned delivery:
  - `VERSION` is the canonical `x.y.z` project value (currently `0.2.28`),
...
- Staged work:
  ...
  - Build-time legacy-content conversion extracts shallow scalar fields with
    ...
    reviewed evidence-coverage gate checks representative IDs and all 26
    indexed special levels. The catalog remains descriptive and never ships a
    Lua runtime or silently infers behavior.
