...
- Deterministic simulation:
  ...
  - Rejected ranged attacks validate target legality and range before consuming
    ...
    and use rejects non-consumable or missing items atomically; focused tests
    assert exact `Game` state preservation for these paths.
...
- Versioned delivery:
  - `VERSION` is the canonical `x.y.z` project value (currently `0.2.94`),
