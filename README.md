...
- Versioned delivery:
  - `VERSION` is the canonical `x.y.z` project value (currently `0.2.45`),
...
- Staged work:
  ...
  - The native `drl-app --mcp` stdio transport has a deterministic JSON-lines
    ...
    capabilities/client metadata, scalar/null request-ID validation, and an
    identified initialize/initialized lifecycle gate before discovery, gameplay,
