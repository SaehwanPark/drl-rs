...
- Versioned delivery:
  - `VERSION` is the canonical `x.y.z` project value (currently `0.2.40`),
...
- Staged work:
  ...
  - The native `drl-app --mcp` stdio transport has a deterministic JSON-lines
    ...
    scenarios, resources, fairness denial, and notification side effects
    without responses; batch and full external-client compatibility remain
    open.
