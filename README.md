...
- Versioned delivery:
  - `VERSION` is the canonical `x.y.z` project value (currently `0.2.116`),
  ...
  - Rejected `Game::step` commands restore turn, world, and RNG state through a
    ...
    invariant.
  - `drl-core::item_definition::CURRENT_ITEM_SPAWN_KINDS` is the single
    representative family catalog used by structural content validation.
