...
- Versioned delivery:
  - `VERSION` is the canonical `x.y.z` project value (currently `0.2.43`),
...
- Staged work:
  ...
  - The native `drl-app --mcp` stdio transport has a deterministic JSON-lines
    contract with version-aware initialize negotiation and an identified
    initialize/initialized lifecycle gate before discovery, gameplay,
