...
- Versioned delivery:
  - `VERSION` is the canonical `x.y.z` project value (currently `0.2.14`),
...
- Staged work:
  ...
  - Release builds emit a hashed static-bundle manifest with graphics rights
    metadata; optional detached manifest signing is supported, while key
    custody and offline/cross-browser acceptance remain open.
