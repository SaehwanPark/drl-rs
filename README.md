...
- Deterministic simulation:
  ...
  - Rejected ranged attacks validate target legality and range before consuming
    ...
    exact `Game` state preservation for these paths; ranged out-of-bounds and
    empty-target rejections are covered as well.
...
- Versioned delivery:
  - `VERSION` is the canonical `x.y.z` project value (currently `0.2.100`),
