...
tag, attributes = by_id.get("minimap-region", (None, {}))
require(tag == "section" and attributes.get("role") == "region", "minimap region role is missing")
require(attributes.get("aria-label") == "Explored minimap", "minimap region is unnamed")
tag, attributes = by_id.get("minimap", (None, {}))
require(tag == "pre", "minimap must be a preformatted text surface")
require(attributes.get("tabindex") is not None, "minimap is not keyboard focusable")
require(attributes.get("aria-live") == "off", "minimap must not be a live region")
